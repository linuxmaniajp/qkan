...
public class QU004 extends QU004Event {
    ...
    public boolean canBack(VRMap parameters) throws Exception {
        ...
        if (getKaigoTableChangeFlg() == 1 || getShahukuTableChangeFlg() == 1
    ...
    protected void kaigoInfoButtonConpileActionPerformed(ActionEvent e)
        ...
        VRBindPathParser.set("PATIENT_ID", map, new Integer(getPatientId()));
        VRBindPathParser.set("KOHI_ID", map,
                VRBindPathParser.get("KOHI_ID", temp));
        VRBindPathParser.set("INSURE_TYPE", map,
                VRBindPathParser.get("INSURE_TYPE", temp));
    ...
    public boolean isValidInput(int insureType, int checkMode) throws Exception {
        ...
        if (insureType == INSURE_TYPE_KAIGO) {
            ...
            int newHistoryIndex = getIndexNewHistory(
                    ACCastUtilities.toInt(VRBindPathParser
                            .get("KOHI_TYPE", map)),
                    ACCastUtilities.toString(VRBindPathParser.get(
    ...
    public void casualInitialize() throws Exception {
        ...
        if (ACFrame.getInstance().hasProperty("ScreenConfig/ShowOldKohi")) {
            showOldKohi = ACCastUtilities.toInt(
                    getProperty("ScreenConfig/ShowOldKohi"), 0);
        }
        // システム日付の取得
    ...
    public VRMap toKohiTableList(VRMap map) throws Exception {
        ...
        if (temp != null) {
            ...
            VRBindPathParser.set("KOHI_PAYER_NAME", map,
                    VRBindPathParser.get("INSURER_NAME", temp));
        }
        ...
        if (temp != null) {
            VRBindPathParser.set("KOHI_NAME", map,
                    VRBindPathParser.get("KOHI_NAME", temp));
        }
        return map;
    ...
    public VRMap toShahukuTableList(VRMap map, String mode) throws Exception {
        ...
        if ("FROM_DB".equals(mode)) {
            ...
            VRBindPathParser.set("BENEFIT_RATE", map,
                    ACCastUtilities.toString((look * 10)));
        }
    ...
    public int getCountDuplicateOnClaimStyle(int claimStyle, VRList reserved)
        ...
        for (int i = 0; i < reserved.size(); i++) {
            ...
            // 公費サービス履歴の件数分ループ
            for (int j = 0; j < getMasterKohiServiceList().size(); j++) {
                VRMap patientKohiService = (VRMap) getMasterKohiServiceList()
                        .get(j);
    ...
    public ArrayList getKohi(ArrayList kohiList, int claimStyle)
        ...
        while (it.hasNext()) {
            ...
            int kohiId = ACCastUtilities.toInt(VRBindPathParser.get("KOHI_TYPE", row));
            for (int i = 0; i < getMasterKohiServiceList().size(); i++) {
                VRMap temp = (VRMap) getMasterKohiServiceList().get(i);
                if (kohiId == ACCastUtilities.toInt(VRBindPathParser.get("KOHI_TYPE", temp))) {
                    int serviceKindDetail = ACCastUtilities.toInt(VRBindPathParser.get("SYSTEM_SERVICE_KIND_DETAIL", temp));
                    
                    VRMap service = (VRMap) VRBindPathParser.get(new Integer(serviceKindDetail), getMasterService());
                    int masterClaimStyle = ACCastUtilities.toInt(VRBindPathParser.get("CLAIM_STYLE_TYPE", service));
