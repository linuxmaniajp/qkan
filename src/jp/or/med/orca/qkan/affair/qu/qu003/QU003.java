...
//					    QkanMessageList.getInstance().UPDATE_SUCCESSED();
   ...
   * @throws Exception 処理例外
   */
  protected void medicalInfoTableSelectionChanged(ListSelectionEvent e) throws Exception{
...
//  	    getMedicalInfoCitiesName().setSelectedItem(null);
   ...
   * @throws Exception 処理例外
   */
  protected void medicalInfoButtonClearActionPerformed(ActionEvent e) throws Exception{
   ...
   * @throws Exception 処理例外
   */
  protected void medicalInfoButtonInsertActionPerformed(ActionEvent e) throws Exception{
	    ...
	    for(int i = 0; i < getMedicalHistoryList().size(); i++){	    	
	    	...
	    	int target = ACCastUtilities.toInt(
	    			VRBindPathParser.get(
	    					"MEDICAL_HISTORY_ID", (VRMap)getMedicalHistoryList().get(i)));
...
//    }
   ...
   * @throws Exception 処理例外
   */
  protected void medicalInfoButtonConpileActionPerformed(ActionEvent e) throws Exception{
...
//    }
    ...
    VRBindPathParser.set(
    		"MEDICAL_HISTORY_ID", map, VRBindPathParser.get("MEDICAL_HISTORY_ID", temp));
   ...
   * @throws Exception 処理例外
   */
  protected void medicalInfoButtonDeleteActionPerformed(ActionEvent e) throws Exception{
    ...
    switch(QkanMessageList.getInstance().WARNING_OF_DELETE_SELECTION()){
    	...
    	case ACMessageBox.RESULT_OK:
    		...
    		getMedicalHistoryList().remove(getMedicalInfoTable().getSelectedModelRow());
   ...
   * @throws Exception 処理例外
   */
  protected void specialInstructButton2ActionPerformed(ActionEvent e) throws Exception{
  ...
  protected void medicalInfoInsurerNoFocusLost(FocusEvent e) throws Exception{
  	...
  	VRMap map = ACBindUtilities.getMatchRowFromValue(
  			getInsurerList(), "INSURER_ID", medicalInsurerId);
   ...
   * @throws Exception 処理例外
   */
  protected void medicalInfoInsurerNameActionPerformed(ActionEvent e) throws Exception{
  ...
  protected void medicalInfoCitiesNoFocusLost(FocusEvent e) throws Exception{
  	...
  	VRMap map = ACBindUtilities.getMatchRowFromValue(
  			getInsurerList(), "INSURER_ID", cityInsurerId);
   ...
   * @throws Exception 処理例外
   */
  protected void medicalInfoCitiesNameActionPerformed(ActionEvent e) throws Exception{
   ...
   * @throws Exception 処理例外
   */
  protected void medicalTreatmentInfoOrganActionPerformed(ActionEvent e) throws Exception{
	...
	if(getMedicalTreatmentInfoOrgan().isSelected()){
		...
		if(getMedicalTreatmentInfoOrgan().getSelectedModelItem() == null 
				|| "".equals(getMedicalTreatmentInfoOrgan().getSelectedModelItem())){
			...
			map = (VRMap)getMedicalTreatmentInfoOrgan().getSelectedModelItem();
		  	
		    // medicalFacilityMapから、以下のフィールドを取得し、結合する。
		    // ・MEDICAL_FACILITY_TEL_FIRST
		    // ・MEDICAL_FACILITY_TEL_SECOND
		    // ・MEDICAL_FACILITY_TEL_THIRD
		  	String tel1 = ACCastUtilities.toString(
		  			VRBindPathParser.get("MEDICAL_FACILITY_TEL_FIRST", map));
		  	String tel2 = ACCastUtilities.toString(
		  			VRBindPathParser.get("MEDICAL_FACILITY_TEL_SECOND", map));
		  	String tel3 = ACCastUtilities.toString(
		  			VRBindPathParser.get("MEDICAL_FACILITY_TEL_THIRD", map));
		    ...
		    // ・MEDICAL_FACILITY_ZIP_SECOND
		  	String zip1 = ACCastUtilities.toString(
		  			VRBindPathParser.get("MEDICAL_FACILITY_ZIP_FIRST", map));
		  	String zip2 = ACCastUtilities.toString(
		  			VRBindPathParser.get("MEDICAL_FACILITY_ZIP_SECOND", map));
		    ...
		    // ・MEDICAL_FACILITY_FAX_THIRD
		  	String fax1 = ACCastUtilities.toString(
		  			VRBindPathParser.get("MEDICAL_FACILITY_FAX_FIRST", map));
		  	String fax2 = ACCastUtilities.toString(
		  			VRBindPathParser.get("MEDICAL_FACILITY_FAX_SECOND", map));
		  	String fax3 = ACCastUtilities.toString(
		  			VRBindPathParser.get("MEDICAL_FACILITY_FAX_THIRD", map));
	...
	// 主治医氏名のソースを退避する。
  	VRMap temp = (VRMap)getMedicalTreatmentInfoMainDoctorName().getSource();
   ...
   * @throws Exception 処理例外
   */
  protected void medicalInfoInsuranceVarietyRadio2SelectionChanged(ListSelectionEvent e) throws Exception{
...
//  	}
   ...
   * @throws Exception 処理例外
   */
  protected void medicalInfoInsuranceVarietyRadio1SelectionChanged(ListSelectionEvent e) throws Exception{
  ...
  public boolean isValidInputStation() throws Exception{
    ...
    if(ACTextUtilities.isNullText(getInstructInfoValidLimit1().getText()) 
    		&& !(ACTextUtilities.isNullText(getInstructInfoValidLimit2().getText()))){
    ...
    if(!(ACTextUtilities.isNullText(getInstructInfoValidLimit2().getText()))){
...
//    }
    ...
    if(!(getMedicalHistoryList() == null || getMedicalHistoryList().size() == 0)){
	    for(int i = 0; i < getMedicalHistoryList().size(); i++){
	    	...
	    	// 編集ボタン押下時
	    	// 選択されている履歴(画面上のデータ自身の履歴)とは比較しない。
	    	...
	    	VRMap map = (VRMap)getMedicalHistoryList().get(i);
	    	
	    	Date reservedStart = ACCastUtilities.toDate(
	    			VRBindPathParser.get("MEDICAL_VALID_START", map));
	    	Date reservedEnd = ACCastUtilities.toDate(
	    			VRBindPathParser.get("MEDICAL_VALID_END", map));
	    	
		    if(ACDateUtilities.
		    		getDuplicateTermCheck(start, end, reservedStart, reservedEnd)
					!= ACDateUtilities.DUPLICATE_NONE){
		    	getMedicalInfoValidLimit1().requestFocus();
			    QkanMessageList.getInstance().QU003_ERROR_OF_DUPLICATE_IRYO();
			    return false;
	    	}
	    }
    }
    ...
    if(getMedicalInfoCitiesLawNo().isEnabled()){
	    if(!ACTextUtilities.isNullText(getMedicalInfoCitiesLawNo().getText())){
    ...
    if(getMedicalInfoOldPersonReceiptNo().isEnabled()){
	    if(!ACTextUtilities.isNullText(getMedicalInfoOldPersonReceiptNo().getText())){
  ...
  public boolean doSave() throws Exception{
    // 保存処理を行う。
  	try{
	    ...
	    // 画面上にないデータを追加する。
	    VRBindPathParser.set("PATIENT_ID", map, new Integer(getPatientId()));
	    VRBindPathParser.set("STATION_HISTORY_ID", map, new Integer(STATION_HISTORY_ID));
  ...
  public void doFind() throws Exception{
    ...
    String familyName = 
    	ACCastUtilities.toString(VRBindPathParser.get("PATIENT_FAMILY_NAME", map));
    String firstName = 
    	ACCastUtilities.toString(VRBindPathParser.get("PATIENT_FIRST_NAME", map));
...
//    getPassiveChecker().reservedPassive(getPASSIVE_CHECK_KEY_STATION(), getStationHistoryList());
  ...
  public void initialize() throws Exception{
	...
	model.setColumns(new String[]{
			"INSURE_TYPE_NAME",
  			"HONNIN_KAZOKU_NAME",
			"MEDICAL_LAW_NO+''+MEDICAL_INSURER_ID",
			"MEDICAL_INSURER_NAME",
			"MEDICAL_INSURE_ID",
			"MEDICAL_VALID_START",
			"MEDICAL_VALID_END",
			"BENEFIT_RATE",
			"CITY_LAW_NO+''+CITY_INSURER_ID",
			"CITY_INSURER_NAME",
			"OLD_RECIPIENT_ID"
  	});
...
//			new Integer[] {new Integer(1), new Integer(2), new Integer(3), }));
    ...
    // 給付割合
    getMedicalInfoTableColumn8().setFormat(new ACHashMapFormat(
			new String[] { "10割", "9割", "8割", "7割" }, 
			new Integer[] {
  ...
  public void casualInitialize() throws Exception{
    ...
    if(!(getInsurerList() == null || getInsurerList().size() == 0)){
	    for(int i = 0; i < getInsurerList().size(); i++){
	    	...
	    	Object insurerId = 
	    		VRBindPathParser.get("INSURER_ID", (VRMap)getInsurerList().get(i));
    ...
    setMedicalFacilityList(QkanCommon.getMedicalFacilityInfo(getDBManager()));
    ...
    // ・KEY：BENEFIT_RATE    
    VRBindPathParser.set("BENEFIT_RATE", map, 
    		QkanCommon.getArrayFromMasterCode(CODE_BENEFIT_RATE, "BENEFIT_RATE"));
  ...
  public VRMap toMedicalTableList(VRMap map) throws Exception{
  	...
  	if(VRBindPathParser.has("MEDICAL_INSURER_ID", map)){
	  	...
	  	temp = ACBindUtilities.getMatchRowFromValue(getInsurerList(), 
	  			"INSURER_ID", VRBindPathParser.get("MEDICAL_INSURER_ID", map));
  	}
  	...
  	if(temp != null){
		...
		// 該当する保険者情報のINSURER_NAMEを渡されたレコードに
  		...
  		VRBindPathParser.set("MEDICAL_INSURER_NAME", 
	...
	// 渡されたレコードのCITY_INSURER_IDの値で、insurerListの
  	...
  	if(temp != null){
		...
		// 該当する保険者情報のINSURER_NAMEを渡されたレコードに
  		...
  		VRBindPathParser.set("CITY_INSURER_NAME", 
  				map, VRBindPathParser.get("INSURER_NAME", temp));
  	}
  	...
  	list = (VRList)VRBindPathParser.get(
  			new Integer(CODE_INSURE_TYPE), getMasterCode());
  	if(VRBindPathParser.has("INSURE_TYPE", map)){
  	  	temp = new VRHashMap();
	  	temp = ACBindUtilities.getMatchRowFromValue(list, 
	  			"CONTENT_KEY", VRBindPathParser.get("INSURE_TYPE", map));
	  	if(temp != null){
	  		VRBindPathParser.set("INSURE_TYPE_NAME", 
  	...
  	list = new VRArrayList();
  	list = (VRList)VRBindPathParser.get(
  			new Integer(CODE_HONNIN_KAZOKU), getMasterCode());
  	if(VRBindPathParser.has("SELF_FLAG", map)){
	  	temp = new VRHashMap();
	  	temp = ACBindUtilities.getMatchRowFromValue(list, 
	  			"CONTENT_KEY", VRBindPathParser.get("SELF_FLAG", map));
	  	if(temp != null){
	  		VRBindPathParser.set("HONNIN_KAZOKU_NAME", 
	  				map, VRBindPathParser.get("CONTENT", temp));
	  	}  	
  	}
