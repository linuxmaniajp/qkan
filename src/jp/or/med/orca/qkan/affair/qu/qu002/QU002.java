...
public class QU002 extends QU002Event {
  ...
  protected void initAction(ACAffairInfo affair) throws Exception {
  	...
  	if(VRBindPathParser.has("COMPULSORY_BACK_FLG", param)){
  		setCompulsoryBackFlg(ACCastUtilities.toInt(
  				VRBindPathParser.get("COMPULSORY_BACK_FLG", param)));
  		// 戻りフラグが1の場合
    ...
    // KEY：PATIENT_ID(利用者ID)
  	if(VRBindPathParser.has("PATIENT_ID", param)){
  		setPatientId(ACCastUtilities.toInt(VRBindPathParser.get("PATIENT_ID", param)));
  	}
  	
    // KEY：PROCESS_MODE(処理モード)
  	if(VRBindPathParser.has("PROCESS_MODE", param)){
  		setProcessMode(ACCastUtilities.toInt(VRBindPathParser.get("PROCESS_MODE", param)));
  	}
  	
    // KEY：PROCESS_START(遷移元画面)
  	if(VRBindPathParser.has("PROCESS_START", param)){
  		setProcessStart(ACCastUtilities.toString(VRBindPathParser.get("PROCESS_START", param)));
  	}else{  	  	
  ...
  public boolean canBack(VRMap parameters) throws Exception {
    ...
    if(getSnapshot().isModified() || 
    		getIdouTableChangeFlg() == 1 ||	getKaigoTableChangeFlg() == 1){
   ...
   * @throws Exception 処理例外
   */
  protected void clearInsertModeActionPerformed(ActionEvent e) throws Exception{
  	...
  	if(getSnapshot().isModified() || 
  		...
  		switch(QkanMessageList.getInstance().WARNING_OF_CLEAR_ON_MODIFIED()){
   ...
   * @throws Exception 処理例外
   */
  protected void clearUpdateModeActionPerformed(ActionEvent e) throws Exception{
    
  	// 変更チェック（スナップショットのチェック、画面テーブルのチェック）
  	if(getSnapshot().isModified() == true || 
    		getIdouTableChangeFlg() == 1 ||	getKaigoTableChangeFlg() == 1){
  	    // 最後に保存されてから、項目・画面テーブルが変更されている場合(下記のいずれかの場合）
  	    // ・isModified
  	    // ・idouTableChangeFlg = 1
   ...
   * @throws Exception 処理例外
   */
  protected void toHokanInfoButtonActionPerformed(ActionEvent e) throws Exception{
    ...
    if(!(msgParam1 == null || "".equals(msgParam1))){
		...
		switch(QkanMessageList.getInstance().QU002_WARNING_OF_TRANSFER(msgParam1, msgParam2)){
   ...
   * @throws Exception 処理例外
   */
  protected void toKohiInfoButtonActionPerformed(ActionEvent e) throws Exception{
   ...
   * @throws Exception 処理例外
   */
  protected void idouInfoTableSelectionChanged(ListSelectionEvent e) throws Exception{
   ...
   * @throws Exception 処理例外
   */
  protected void idouInfoButtonClearActionPerformed(ActionEvent e) throws Exception{
   ...
   * @throws Exception 処理例外
   */
  protected void idouInfoButtonInsertActionPerformed(ActionEvent e) throws Exception{
	    ...
	    for(int i = 0; i < getIdouHistoryList().size(); i++){
	    	...
	    	int target = ACCastUtilities.toInt(
	    			VRBindPathParser.get(
	    					"CHANGES_HISTORY_ID", (VRMap)getIdouHistoryList().get(i)));
   ...
   * @throws Exception 処理例外
   */
  protected void idouInfoButtonEditActionPerformed(ActionEvent e) throws Exception{
    ...
    VRBindPathParser.set(
    		"CHANGES_HISTORY_ID", map, VRBindPathParser.get("CHANGES_HISTORY_ID", temp));
   ...
   * @throws Exception 処理例外
   */
  protected void idouInfoButtonDeleteActionPerformed(ActionEvent e) throws Exception{
  	...
  	if(!(getIdouHistoryList() == null || getIdouHistoryList().size() == 0)){
	    ...
	    switch(QkanMessageList.getInstance().WARNING_OF_DELETE_SELECTION()){
	    	...
	    	case ACMessageBox.RESULT_OK:
			    ...
			    getIdouHistoryList().remove(getIdouInfoTable().getSelectedModelRow());			
  ...
  protected void kaigoInfoButtonInsertActionPerformed(ActionEvent e) throws Exception{
    ...
    if(!(getKaigoHistoryList() == null || getKaigoHistoryList().size() == 0)){
	    ...
	    for(int i = 0; i < getKaigoHistoryList().size(); i++){	    	
	    	...
	    	int target = ACCastUtilities.toInt(
	    			VRBindPathParser.get(
	    					"NINTEI_HISTORY_ID", (VRMap)getKaigoHistoryList().get(i)));
...
//    }
   ...
   * @throws Exception 処理例外
   */
  protected void kaigoInfoButtonEditActionPerformed(ActionEvent e) throws Exception{
    ...
    VRBindPathParser.set("PATIENT_ID", map, new Integer(getPatientId()));
    VRBindPathParser.set(
    		"NINTEI_HISTORY_ID", map, VRBindPathParser.get("NINTEI_HISTORY_ID", temp));
...
//    }
   ...
   * @throws Exception 処理例外
   */
  protected void kaigoInfoDeleteActionPerformed(ActionEvent e) throws Exception{
  	...
  	if(!(getKaigoHistoryList() == null || getKaigoHistoryList().size() == 0)){
    	...
    	case ACMessageBox.RESULT_OK:
    		...
    		if(jotaiCode != YOUKAIGODO_HIGAITOU){
			    ...
			    VRMap param = new VRHashMap();
	    		VRBindPathParser.set("PATIENT_ID", param, new Integer(getPatientId()));
	    		VRBindPathParser.set("VALID_START", param, VRBindPathParser.get("INSURE_VALID_START", map));
	    		VRBindPathParser.set("VALID_END", param, VRBindPathParser.get("INSURE_VALID_END", map));
			    ...
			    if(!(list == null || list.size() == 0)){				
			    	...
			    	String msgParam = "編集";
				    switch(QkanMessageList.getInstance().QU002_WARNING_OF_OUT_OF_VALID(msgParam)){
				    case ACMessageBox.RESULT_CANCEL:
				    	return;	
				    }
			    }
	    	...
	    	getKaigoHistoryList().remove(getKaigoInfoTable().getSelectedModelRow());
   ...
   * @throws Exception 処理例外
   */
  protected void idouInfoServiseActionPerformed(ActionEvent e) throws Exception{
    ...
    // 選択されたサービスにより異動事由コンボ（idouInfoChangeContent）に下記のCODE_IDの選択枝を設定する。
  	...
  	if(map != null){
	  	...
	  	switch(ACCastUtilities.toInt(VRBindPathParser.get("CHANGES_CONTENT_TYPE", map))){
   ...
   * @throws Exception 処理例外
   */
  protected void idouInfoChangeContentActionPerformed(ActionEvent e) throws Exception{
    
  	String caption = null;
  	VRList list = new VRArrayList();
  	
  	VRMap service = (VRMap)getIdouInfoServise().getSelectedModelItem();
  	VRMap content = (VRMap)getIdouInfoChangeContent().getSelectedModelItem();
  	...
  	if(service != null && content != null){
	  	...
	  	switch(ACCastUtilities.toInt(
	  			VRBindPathParser.get("CHANGES_CONTENT_TYPE", service))){
   ...
   * @throws Exception 処理例外
   */
  protected void idouInfoReasonActionPerformed(ActionEvent e) throws Exception{
  	...
  	// 異動事由の種類
  	int contentType = ACCastUtilities.toInt(
  			VRBindPathParser.get("CHANGES_CONTENT_TYPE", service));
  	// 理由/状況の値
	int reasonValue = ACCastUtilities.toInt(
			VRBindPathParser.get("CHANGES_REASON", reason));
  ...
  protected void kaigoInfoInsurerIdFocusLost(FocusEvent e) throws Exception{
	...
	VRMap map = (VRMap)getKaigoInfoInsurerName().getDataFromBindPath(insurerId);
   ...
   * @throws Exception 処理例外
   */
  protected void kaigoInfoInsurerNameActionPerformed(ActionEvent e) throws Exception{
    ...
    // 選択された保険者の保険者番号を保険者番号フィールド（kaigoInfoInsurerId）
  	...
  	if(VRBindPathParser.get("INSURER_ID", map) == null){
  		...
  		getKaigoInfoInsurerId().setText(ACCastUtilities.toString(VRBindPathParser.get("INSURER_ID", map)));
  	}
   ...
   * @throws Exception 処理例外
   */
  protected void kaigoInfoKyotakuServiceRadioSelectionChanged(ListSelectionEvent e) throws Exception{
    
  	if(getKaigoInfoKyotakuServiceRadio().getSelectedIndex() == 1){
   ...
   * @throws Exception 処理例外
   */
  protected void kaigoInfoYokaigoInfoActionPerformed(ActionEvent e) throws Exception{
  ...
  protected void basicInfoBirthFocusLost(FocusEvent e) throws Exception{
	...
	VRMap map = new VRHashMap();
  	...
  	if(ACTextUtilities.isNullText(getBasicInfoBirth().getText())){
	  	...
	  	// 正しく入力されている場合
	  	VRBindPathParser.set("PATIENT_BIRTHDAY", map, getBasicInfoBirth().getDate());
  	}
   ...
   * @throws Exception 処理例外
   */
  protected void institutionInfoTokuteiNyushoActionPerformed(ActionEvent e) throws Exception{
...
//    param.setData("PATIENT_ID", new Integer(10));
    param.setData("PROCESS_MODE", new Integer(QkanConstants.PROCESS_MODE_INSERT));
  ...
  public boolean isValidInputPatient() throws Exception{
    ...
    // ※エラーの場合、String：msgParamを宣言し、"氏名"を代入する。  	
  	if(ACTextUtilities.isNullText(getBasicInfoName1().getText())
  			...
  			msgParam = "氏名";
  			QkanMessageList.getInstance().ERROR_OF_NEED_CHECK_FOR_INPUT(msgParam);  			
  			return false;  		
  	}
    ...
    // ※エラーの場合、String：msgParamを宣言し、"ふりがな"を代入する。  	
  	if(ACTextUtilities.isNullText(getBasicInfoFurigana1().getText()) 
  			&& ACTextUtilities.isNullText(getBasicInfoFurigana2().getText())){
    ...
    // ※エラーの場合、String：msgParamを宣言し、"性別"を代入する。  	
  	if(!getBasicInfoSexs().isSelected()){
  		...
  		msgParam = "性別"; 
  		QkanMessageList.getInstance().ERROR_OF_NEED_CHECK_FOR_SELECT(msgParam);
  		return false;
  	}  	
    ...
    // ※エラーの場合、String：msgParamを宣言し、"生年月日に"を代入する。
  	...
  	if(getInstitutionInfoTokuteiNyusho().isSelected()){
  		...
  		if(ACTextUtilities.isNullText(getInstitutionInfoDinnerBearLimitMoneyText().getText())){
  	    ...
  	    // ※エラーの場合、String：msgParamを宣言し、"負担限度額"を代入する。
  		if(ACTextUtilities.isNullText(getInstitutionInfoUnitRoomLimitMoneyText().getText())){
  	    ...
  	    // ※エラーの場合、String：msgParamを宣言し、"負担限度額"を代入する。 
  		if(ACTextUtilities.isNullText(getInstitutionInfoUnitSemiRoomLimitMoneyText().getText())){
  	    ...
  	    // ※エラーの場合、String：msgParamを宣言し、"負担限度額"を代入する。 
  		if(ACTextUtilities.isNullText(getInstitutionInfoNormalRoomLimitMoneyText().getText())){
  	    ...
  	    // ※エラーの場合、String：msgParamを宣言し、"負担限度額"を代入する。 
  		if(ACTextUtilities.isNullText(getInstitutionInfoNormalRoomLimitMoneyText2().getText())){
  	    ...
  	    // ※エラーの場合、String：msgParamを宣言し、"負担限度額"を代入する。  
  		if(ACTextUtilities.isNullText(getInstitutionInfoTasyoRoomLimitMoneyText().getText())){
  ...
  public int getLimitRate() throws Exception{
	...
	if(param != null){
	    ...
	    if(list == null || list.size() == 0){
		    ...
		    if(!(list == null || list.size() == 0)){
			    limitRate = ACCastUtilities.toInt(
			    		VRBindPathParser.get("LIMIT_RATE_VALUE", (VRMap)list.get(0)));
		    }
  ...
  public VRMap getParamForLimitRate() throws Exception{
	...
	// ・kaigoInfoValidLimit3（有効期間終了）　KEY：VALID_END
  	VRMap param = new VRHashMap();
  	VRBindPathParser.set("INSURER_ID", param, getKaigoInfoInsurerId().getText());
  	VRMap yokaigo = (VRMap)getKaigoInfoYokaigoInfo().getSelectedModelItem();
	VRBindPathParser.set("JOTAI_CODE", param, VRBindPathParser.get("JOTAI_CODE", yokaigo));
	VRBindPathParser.set("VALID_START", param, getKaigoInfoValidLimit1().getDate());
	VRBindPathParser.set("VALID_END", param, getKaigoInfoValidLimit3().getDate());
  ...
  public void doFindExternalUseLimit() throws Exception {
		...
		getKaigoInfoExternalUseLimit().setText(ACCastUtilities.toString(externalUseLimit));
  ...
  public int getExternalUseLimit() throws Exception{
		...
		if(param != null){
		    ...
		    if(!(list == null || list.size() == 0)){
		    	// 外部利用型上限単位数を設定する。
		    	externalUseLimit = ACCastUtilities.toInt(
		    			VRBindPathParser.get("LIMIT_RATE_VALUE", (VRMap)list.get(0)));
		    }
...
//	    }
	    ...
	    int benefitRate = ACCastUtilities.toInt(getKaigoInfoBenefit().getText());
	    ...
	    // 給付率
	    int insureRate = ACCastUtilities.toInt(getKaigoInfoBenefit().getText(), 0);
		    
	    if(insuredId.charAt(0) != 'H'){
	    	if(insureRate == INSURE_RATE_SEIHO){
	    		// 生保単独でない時に給付率が「0%」と設定されていた場合
	    		QkanMessageList.getInstance().QU002_ERROR_OF_RATE_FOR_NOT_SEIHO_TANDOKU();
	    		return false;
	    	}
	    }
...
//		    	msgParam1 = "居宅介護支援事業所";
//		  		QkanMessageList.getInstance().ERROR_OF_NEED_CHECK_FOR_SELECT(msgParam1);
		        QkanMessageList.getInstance().QU002_ERROR_OF_NO_DATA_SHIEN_PROVIDER();
		    	return false;  		
		  	}
	  	}
    ...
    int targetIndex = -1;	// 編集対象となる履歴のインデックス
  	...
  	if(getNonCorrespondenceFlg() == 0){
	    ...
	    if(ACDateUtilities.getDifferenceOnDay(start, end) > 0){
	    	...
	    	msgParam2 = "開始日";
	    	msgParam3 = "終了日";
	        QkanMessageList.getInstance().ERROR_OF_GREATER_DATE_RELATION(msgParam1, msgParam2, msgParam3);
	        return false;    	
	    }
	    ...
	    if(!(getKaigoHistoryList() == null || getKaigoHistoryList().size() == 0)){
	    	...
	    	for(int i = 0; i < getKaigoHistoryList().size(); i++){		// 認定履歴件数分ループ
	    		...
	    		// 対象の履歴が非該当の場合は無視する。
	    		int targetJotaiCode = ACCastUtilities.toInt(VRBindPathParser.get("JOTAI_CODE", map));
	    		if(targetJotaiCode == YOUKAIGODO_HIGAITOU){
	    			continue;
	    		}
	    		
	    		reservedStart = (Date)VRBindPathParser.get("INSURE_VALID_START", map);		// 履歴の有効期間開始日
	    		if(latestStart == null){
	    			latestStart = reservedStart;
	    			targetIndex = i;
	    		}else if(ACDateUtilities
	    				.getDifferenceOnDay(latestStart, reservedStart) < 0){
	    			latestStart = reservedStart;
	    			targetIndex = i;
	    		}
	    	}
	    	...
	    	for(int i = 0; i < getKaigoHistoryList().size(); i++){		// 認定履歴件数分ループ
	    		...
	    		reservedStart = (Date)VRBindPathParser.get("INSURE_VALID_START", map);
	    		reservedEnd = (Date)VRBindPathParser.get("INSURE_VALID_END", map);
	    		
	    		// 期間重複チェック
		    	// チェックの結果を取得
	    		int result = ACDateUtilities
						.getDuplicateTermCheck(reservedStart, reservedEnd, start, end);
	    		
	    		if(result != ACDateUtilities.DUPLICATE_NONE){
	    			// 重複していた場合
	    			
	    			getKaigoInfoValidLimit1().requestFocus();
	    			QkanMessageList.getInstance().QU002_ERROR_OF_DUPLICATE_KAIGO();
	    			return false;
	    	...
	    	if(targetIndex != -1){
		    	VRMap latestMap = (VRMap)getKaigoHistoryList().get(targetIndex);		// 最後の履歴抽出
		    	reservedStart = (Date)VRBindPathParser.get("INSURE_VALID_START", latestMap);		// 最後の履歴の有効期間開始日
		    	reservedEnd = (Date)VRBindPathParser.get("INSURE_VALID_END", latestMap);			// 最後の履歴の有効期間終了日
		    	...
		    	if(result != ACDateUtilities.DUPLICATE_NONE){
	    			...
	    			if(result == ACDateUtilities.DUPLICATE_FIRST_END_AND_SECOND_BEGIN){
			    		// 有効期間を自動で調整できる場合
					    // 確認メッセージを表示する。
					    // ・メッセージID：QU002_WARNING_OF_DUPLICATE_KAIGO
					    switch(QkanMessageList.getInstance().QU002_WARNING_OF_DUPLICATE_KAIGO()){
						    case ACMessageBox.RESULT_OK:
						    	// 「OK」が選択された場合
  	...
  	if(getNonCorrespondenceFlg() == 0){		// 非該当の場合は処理を通らない。
	    ...
	    if(getKaigoInfoInterruptionReasonRadio().isSelected()){
		    if(ACTextUtilities.isNullText(getKaigoInfoInterruptionDate().getText())){
		      	// 入力されているかどうか    
		    ...
		    // 検索して取得した認定履歴の件数分ループする。
		  	for(int i = 0; i < getKaigoHistoryList().size(); i++){
	    		...
	    		if(targetJotaiCode == YOUKAIGODO_HIGAITOU){
		  		...
		  		if(ACDateUtilities.getDifferenceOnMonth(start, reservedStart) == 0 
		  				|| ACDateUtilities.getDifferenceOnMonth(start, reservedEnd) == 0){
				    // 履歴の被保険者番号とkaigoInfoInsuredId（被保険者番号）に
		  			// 入力されている被保険者番号が異なる場合
		  			if(!insuredId.equals(ACCastUtilities.toString(VRBindPathParser.get("INSURED_ID", map)))){
		  				// 数値型変数numに1を加える。
		  				num += 1;
		  			}
				    ...
				    // numの値が2の場合
		  			if(num == 2){
		  				getKaigoInfoInsuredId().requestFocus();		// フォーカス
					    // エラーメッセージを表示する。
					    QkanMessageList.getInstance().QP002_ERROR_OF_INSURED_ID();
					    return false;
		  			}
    ...
    if(checkMode == CHECK_MODE_UPDATE){    	
	    ...
	    if(getProcessMode() == QkanConstants.PROCESS_MODE_UPDATE){
		    ...
		    // 対象の認定の要介護度を退避
		    int targetJotaiCode = ACCastUtilities.toInt(VRBindPathParser.get("JOTAI_CODE", temp));
		    ...
		    if(targetJotaiCode != YOUKAIGODO_HIGAITOU){
			    ...
			    VRBindPathParser.set("VALID_START", param, VRBindPathParser.get("INSURE_VALID_START", temp));
			    VRBindPathParser.set("VALID_END", param, VRBindPathParser.get("INSURE_VALID_END", temp));
		    }
		    	...
		    	if(getNonCorrespondenceFlg() == 0){
				    ...
				    if(!(oldList == null || oldList.size() == 0)){
					    ...
					    for(int i = 0; i < oldList.size(); i++){
					    	...
					    	int index = ACBindUtilities.getMatchIndexFromMap(newList, "SERVICE_ID", (VRMap)oldList.get(i));
					    	...
					    	if(index == -1){		    	
					    		...
					    		msgParam1 = "編集";
					    		switch(QkanMessageList.getInstance().QU002_WARNING_OF_OUT_OF_VALID(msgParam1)){
					    		case ACMessageBox.RESULT_OK:
					    			stopFlg = true;
					    			break;
					    		case ACMessageBox.RESULT_CANCEL:
    ...
    if(getNonCorrespondenceFlg() == 0){
	    ...
	    if(insuredId.charAt(0) == 'H'){
	    	...
	    	if(insureRate != INSURE_RATE_SEIHO){
	    		...
	    		if (QkanMessageList.getInstance()
						.QU002_WARNING_OF_CHANGE_RATE_FOR_SEIHO_TANDOKU() == ACMessageBox.RESULT_YES) {
		    		// 給付率を「0%」に設定する。
	    			getKaigoInfoBenefit().setText(ACCastUtilities.toString(INSURE_RATE_SEIHO));
	    		}
	    	}
			// 公費登録をうながすメッセージを表示する。	
	    	// ・メッセージID：QU002_REQUEST_TO_INSERT_SEIHO
	    ...
	    if(doUpdateFlg == 1){
	    	...
	    	VRBindPathParser.set(
	    			"INSURE_VALID_END", (VRMap)getKaigoHistoryList().get(targetIndex), changedDate);
	    }
    }
...
//		    VRBindPathParser.set("PATIENT_ID", data, new Integer(getPatientId()));
		    ...
		    // 利用者IDを取得し、patientIdに格納する。
		    setPatientId(ACCastUtilities.toInt(VRBindPathParser.get("PATIENT_ID", temp)));
		    ...
		    getPassiveChecker().addPassiveUpdateTask(getPASSIVE_CHECK_KEY_PATIENT(), 0);
	    ...
	    // 要介護認定履歴情報
  		...
  		if(getKaigoDataFlg() == 1){
		    ...
		    // ・KEY：PATIENT_ID VALUE：patientId
  			param = new VRHashMap();
  			VRBindPathParser.set(
  					"PATIENT_ID", param, new Integer(getPatientId()));
    ...
    // レコード取得・画面設定を行う。
  	...
  	if(getProcessMode() == QkanConstants.PROCESS_MODE_UPDATE){
	    ...
	    if(getPatientInfoList().size() > 0){
	    	getBasicInfoAndInfoButton().setSource((VRMap)getPatientInfoList().get(0));
	    }
	    
	    // 取得した施設履歴情報（shisetsuHistoryList）の最初のレコードを抽出し、
	    // 「クライアント領域(contents)」のsourceに設定する。
	    if(getShisetsuHistoryList().size() > 0){
	    	getInstitutionInfos().setSource((VRMap)getShisetsuHistoryList().get(0));
	    }
  ...
  public void doFindPatient() throws Exception{
    // 利用者情報取得を行う。
  	...
  	if(getProcessMode() == QkanConstants.PROCESS_MODE_UPDATE){
	    ...
	    setPatientInfoList(QkanCommon.getPatientInfo(getDBManager(), getPatientId()));
	    ...
	    if(getPatientInfoList().size() == 0){
	    	...
	    	getPassiveChecker().reservedPassive(getPASSIVE_CHECK_KEY_PATIENT(), getPatientInfoList());
	    }
  	}
...
//    getIdouInfoTableColumn5().setFormat(new QkanChangesReasonFormat());
	...
	// 郵便番号から住所変換
    // 郵便番号と住所を関連付ける。
	new ACZipRelation(getBasicInfoZip1(),getBasicInfoZip3(),getBasicInfoAddress());
...
//    }
  ...
  public void casualInitialize() throws Exception{
    ...
    VRBindPathParser.set(
    		"JOTAI_CODE", map, QkanCommon.getArrayFromMasterCode(CODE_YOKAIGODO, "JOTAI_CODE"));
...
//    		"INSURE_RATE", map, QkanCommon.getArrayFromMasterCode(CODE_INSURE_RATE, "INSURE_RATE"));
    ...
    setMasterService(QkanCommon.getMasterService(getDBManager(), TARGET_DATE_20060401));
    
    VRList temp = new VRArrayList(
    		getMasterService().values());
    
    if(!(temp == null || temp.size() == 0)){
    	for(int i = 0; i < temp.size(); i++){
    		VRMap serviceMap = (VRMap)temp.get(i);
    		int systemServiceKindDetail = ACCastUtilities.toInt(
    				VRBindPathParser.get("SYSTEM_SERVICE_KIND_DETAIL", serviceMap));
    		
    	    // 「その他」「主な日常生活上の活動」を排除
    		if(!(systemServiceKindDetail == SERVICE_TYPE_OTHER 
    				|| systemServiceKindDetail == SERVICE_TYPE_ROUTINE)){
    ...
    // 居宅介護支援事業所情報を取得し、以下のKEYでmapに設定する。
    // ・KEY：PROVIDER
    ...
    // ・KEY：INSURER
    VRBindPathParser.set(
    		"INSURER", map, QkanCommon.getInsurerInfoCareOnly(getDBManager()));
  ...
  public VRMap toKaigoTableList(VRMap map) throws Exception{
  	...
  	list = (VRList)VRBindPathParser.get(
  			new Integer(CODE_YOKAIGODO), getMasterCode());
  	temp = new VRHashMap();  	
  	if(VRBindPathParser.has("JOTAI_CODE", map)){
	  	temp = ACBindUtilities.getMatchRowFromValue(list, 
	  			"CONTENT_KEY", VRBindPathParser.get("JOTAI_CODE", map));
	  	if(temp != null){
	  		VRBindPathParser.set("JOTAI_CODE_NAME", 
	  				map, VRBindPathParser.get("CONTENT", temp));
	  	}
  	}
  	...
  	list = new VRArrayList();
  	list = (VRList)VRBindPathParser.get(
  			new Integer(CODE_SHINSEI_KUBUN), getMasterCode());
  	temp = new VRHashMap();  	
  	if(VRBindPathParser.has("SHUBETSU_CODE", map)){
	  	temp = ACBindUtilities.getMatchRowFromValue(list, 
	  			"CONTENT_KEY", VRBindPathParser.get("SHUBETSU_CODE", map));
	  	if(temp != null){
	  		VRBindPathParser.set("SHUBETSU_CODE_NAME", 
	  				map, VRBindPathParser.get("CONTENT", temp));
	  	}
  	}
  	...
  	list = new VRArrayList();
  	list = (VRList)VRBindPathParser.get(
  			new Integer(CODE_TYUDAN_RIYU), getMasterCode());
  	temp = new VRHashMap();  	
  	if(VRBindPathParser.has("STOP_REASON", map)){
	  	temp = ACBindUtilities.getMatchRowFromValue(list, 
	  			"CONTENT_KEY", VRBindPathParser.get("STOP_REASON", map));
	  	if(temp != null){
	  		VRBindPathParser.set("STOP_REASON_NAME", 
	  				map, VRBindPathParser.get("CONTENT", temp));
	  	}
  	}
  ...
  public VRMap toIdouTableList(VRMap map) throws Exception{
  	...
  	// 渡された異動情報のサービス種類コード
  	int serviceType = ACCastUtilities.toInt(
  			VRBindPathParser.get("SYSTEM_SERVICE_KIND_DETAIL", map));

  	// サービス種類コードを用いて、サービスデータを取得
  	VRMap service = (VRMap)VRBindPathParser.get(
  			new Integer(serviceType), getMasterService());
  	...
  	String serviceName = ACCastUtilities.toString(
  			VRBindPathParser.get("SERVICE_ABBREVIATION", service));
  	...
  	if(!(VRBindPathParser.get("CHANGES_CONTENT", map) == null 
  		...
  		content = ACCastUtilities.toInt(VRBindPathParser.get("CHANGES_CONTENT", map));
  	}
  	...
  	if(!(VRBindPathParser.get("CHANGES_REASON", map) == null 
  		...
  		reason = ACCastUtilities.toInt(VRBindPathParser.get("CHANGES_REASON", map));
  	}
  	...
  	switch(contentType){
  		...
  		case IDOU_SERVICE_KYOTAKU:
			...
			codeContentList = (VRList)VRBindPathParser.get(
					new Integer(CODE_IDOU_CONTENT_KYOTAKU), getMasterCode());
			...
			VRBindPathParser.set("CONTENT", map, VRBindPathParser.get("CONTENT", temp));
			
  			if(content == IDOU_STOP_KYOTAKU){
  				
  				// コードマスタより居宅系異動理由のデータ取得
  				codeReasonList = new VRArrayList();
  				codeReasonList = (VRList)VRBindPathParser.get(
  						new Integer(CODE_IDOU_REASON_KYOTAKU), getMasterCode());
		...
		case IDOU_SERVICE_SHISETSU1:
			...
			codeContentList = new VRArrayList();
			codeContentList = (VRList)VRBindPathParser.get(
					new Integer(CODE_IDOU_CONTENT_SHISETSU1), getMasterCode());
			...
			VRBindPathParser.set("CONTENT", map, VRBindPathParser.get("CONTENT", temp));
  			...
  			if(content == IDOU_STOP_SHISETSU1){
  				...
  				codeReasonList = new VRArrayList();
  				codeReasonList = (VRList)VRBindPathParser.get(
  						new Integer(CODE_IDOU_REASON_SHISETSU1), getMasterCode());
		...
		case IDOU_SERVICE_SHISETSU2:
			...
			codeContentList = new VRArrayList();
			codeContentList = (VRList)VRBindPathParser.get(
					new Integer(CODE_IDOU_CONTENT_SHISETSU2), getMasterCode());
			...
			VRBindPathParser.set("CONTENT", map, VRBindPathParser.get("CONTENT", temp));
  			...
  			if(content == IDOU_STOP_SHISETSU2){
  				...
  				codeReasonList = new VRArrayList();
  				codeReasonList = (VRList)VRBindPathParser.get(
  						new Integer(CODE_IDOU_REASON_SHISETSU2), getMasterCode());
		...
		case IDOU_SERVICE_IRYO_KANGO:
			...
			codeContentList = new VRArrayList();
			codeContentList = (VRList)VRBindPathParser.get(
					new Integer(CODE_IDOU_CONTENT_IRYO_KANGO), getMasterCode());
			...
			VRBindPathParser.set("CONTENT", map, VRBindPathParser.get("CONTENT", temp));
  			...
  			if(content == IDOU_STOP_IRYO_KANGO){
  				...
  				codeReasonList = new VRArrayList();
  				codeReasonList = (VRList)VRBindPathParser.get(
  						new Integer(CODE_IDOU_REASON_IRYO_KANGO), getMasterCode());
		...
		case IDOU_SERVICE_SHISETSU3:
			...
			codeContentList = new VRArrayList();
			codeContentList = (VRList)VRBindPathParser.get(
					new Integer(CODE_IDOU_CONTENT_SHISETSU3), getMasterCode());
			...
			VRBindPathParser.set("CONTENT", map, VRBindPathParser.get("CONTENT", temp));
  			...
  			if(content == IDOU_STOP_IRYO_KANGO){
  				...
  				codeReasonList = new VRArrayList();
  				codeReasonList = (VRList)VRBindPathParser.get(
  						new Integer(CODE_IDOU_REASON_SHISETSU3), getMasterCode());
  ...
  public void displayNowYokaigodo() throws Exception{
	...
	if(target == null || target.size() == 0){
		...
		for(int i = 0; i < target.size(); i++){
			...
			Date start = ACCastUtilities.toDate(VRBindPathParser.get("INSURE_VALID_START", temp));
			Date end = ACCastUtilities.toDate(VRBindPathParser.get("INSURE_VALID_END", temp));			
			if(isValidTermOnTargetDate(getSystemDate(), start, end)){
				// システム日付において有効な認定履歴である場合
				int yokaigodo = ACCastUtilities.toInt(VRBindPathParser.get("JOTAI_CODE", temp));
				if(yokaigodo == YOUKAIGODO_HIGAITOU){
					// 非該当の場合は一時保留
					hasHigaito = true;
				}else{
					// 非該当でない場合はループ終了
					yokaigodoNow = ACCastUtilities.toString(VRBindPathParser.get("JOTAI_CODE_NAME", temp));
					break;
				}
			}
		}
   ...
   * @throws Exception 処理例外
   */
  public boolean isValidTermOnTargetDate(Date targetDate, Date start, Date end) throws Exception{
