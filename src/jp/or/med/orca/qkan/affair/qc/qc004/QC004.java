...
public class QC004 extends QC004Event {
    ...
    protected void initAction(ACAffairInfo affair) throws Exception {
        ...
        setPASSIVE_CHECK_KEY_KIROKUSHO(new ACPassiveKey("HOMONKANGO_KIROKUSHO",
        ...
        patientList = (VRArrayList) QkanCommon.getPatientInfo(getDBManager(),
                getPatientId());
        ...
        String patientName = QkanCommon.toFullName(
                ACCastUtilities.toString(getPatientMap().getData(
                        "PATIENT_FAMILY_NAME")),
                ACCastUtilities.toString(getPatientMap().get(
                        "PATIENT_FIRST_NAME")));

        // patientAgeにレコードの生年月日からシステム日付を基準にして年齢を算出した結果を格納する。
        int patientAge = QkanCommon.calcAge(ACCastUtilities
                .toDate(getPatientMap().getData("PATIENT_BIRTHDAY")));
    ...
    protected void openActionPerformed(ActionEvent e) throws Exception {
        ...
        if (QkanMessageList.getInstance()
            ...
            param.setData("TARGET_DATE",
                    ACDateUtilities.toFirstDayOfMonth(getTargetDate()));

            // SQL文を取得する。
            // 取得したSQL文を実行する。
            VRList lastKirokushoList = getDBManager().executeQuery(
                    getSQL_GET_LAST_HOMONKANGO_KIROKUSHO(param));
    ...
    protected void printActionPerformed(ActionEvent e) throws Exception {
        ...
        // if (QkanMessageList.getInstance().WARNING_OF_PRINT("訪問看護記録書I") ==
        ...
        // 訪問看護記録書IのNO.2を作成する。
        // 帳票を出力する。
        if (qc00401.doPrint(writer, printParameter)
                && qc00402.doPrint(writer, printParameter)) {
     ...
     * @throws Exception 処理例外
     */
    protected void syujiMedicalNameActionPerformed(ActionEvent e)
            throws Exception {
        ...
        VRMap selectedMap = (VRMap) getSyujiMedicalName()
    ...
    public void doFindPassive() throws Exception {
        ...
        VRList houmonKangoKirokushoList = getDBManager().executeQuery(
                getSQL_GET_HOMONKANGO_KIROKUSHO(param));
        ...
        if (!houmonKangoKirokushoList.isEmpty()) {
            ...
            VRMap houmonKangoKirokushoMap = (VRMap) houmonKangoKirokushoList
                    .getData(0);
            ...
            getPassiveChecker().reservedPassive(
                    getPASSIVE_CHECK_KEY_KIROKUSHO(), getKirokushoList());
    ...
    public boolean doInsert() throws Exception {
        ...
        try {
            ...
            // KEY：TARGET_DATE VALUE：targetDate
            getKirokushoMap()
                    .setData("PATIENT_ID", new Integer(getPatientId()));
            getKirokushoMap().setData("TARGET_DATE", getTargetDate());

            // パッシブタスクを登録する。
            getPassiveChecker().addPassiveInsertTask(
                    getPASSIVE_CHECK_KEY_KIROKUSHO(), getKirokushoMap());
            ...
            if (getPassiveChecker().passiveCheck(getDBManager())) {
                ...
                getDBManager().executeUpdate(
                        getSQL_INSERT_KIROKUSHO(getKirokushoMap()));
    ...
    public boolean doUpdate() throws Exception {
        ...
        try {
            ...
            getPassiveChecker().addPassiveUpdateTask(
                    getPASSIVE_CHECK_KEY_KIROKUSHO());
            ...
            if (getPassiveChecker().passiveCheck(getDBManager())) {
                ...
                // 取得したSQL文を実行する。
                getDBManager().executeUpdate(
                        getSQL_UPDATE_KIROKUSHO(getKirokushoMap()));
    ...
    public void setParameter(VRMap parameters) throws Exception {
        ...
        ACAffairInfo affair = new ACAffairInfo(QC004.class.getName(),
                parameters);
    }
    ...
    public void getMedicalFacilityInfo() throws Exception {
        ...
        // 取得したSQL文を実行する。
        VRList hospitalList = getDBManager().executeQuery(
                getSQL_GET_MEDICAL_FACILITY(null));
