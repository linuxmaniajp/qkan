 ...
 * サブシステム 予定管理 (S)
 * プロセス サービス予定 (001)
 * プログラム サービスパターン訪問介護 (QS001101_H2104)
 *
 *****************************************************************
 */
...
/**
 * サービスパターン訪問介護画面項目デザイン(QS001101_H2104) 
 */
public class QS001101_H2104Design extends QS001ServicePanel {
  //GUIコンポーネント
  ...
  protected void addHoumonKaigoPatterns(){
    ...
    houmonKaigoPatterns.add(getHoumonKaigoFirstAddRadioGroupContainer(), VRLayout.FLOW_INSETLINE_RETURN);

    houmonKaigoPatterns.add(getHoumonKaigoEmergencyAddRadioGroupContainer(), VRLayout.FLOW_INSETLINE_RETURN);

    houmonKaigoPatterns.add(getProviderAddMountainousAreaRafioRadioGroupContainer(), VRLayout.FLOW_INSETLINE_RETURN);
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      ACFrame.debugStart(new ACAffairInfo(QS001101_H2104Design.class.getName()));
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
   ...
   * 自身を返します。
   */
  protected QS001101_H2104Design getThis() {
    return this;
  }
}
