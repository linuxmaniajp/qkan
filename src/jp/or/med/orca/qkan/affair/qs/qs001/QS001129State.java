 ...
 * アプリ: QKANCHO
 * 開発者: 田中　統蔵
 * 作成日: 2006/03/15  日本コンピューター株式会社 田中　統蔵 新規作成
 * 更新日: ----/--/--
 * システム 給付管理台帳 (Q)
 * サブシステム 予定管理 (S)
 * プロセス サービス予定 (001)
...
/**
 * サービスパターン地域密着型介護福祉施設状態定義(QS001129) 
 */
public class QS001129State extends QS001129Design {
  /**
   * コンストラクタです。
   */
  public QS001129State(){
  }
  ...
  /**
   * 「重度化対応加算・あり」の状態に設定します。
   * @throws Exception 処理例外
   */
  public void setState_VALID_HEAVILY() throws Exception {

        getKaigoWelfareFacilityHeavilyCorrespondenceAddRadio().setEnabled(true);

  }

  /**
   * 「重度化対応加算・なし」の状態に設定します。
   * @throws Exception 処理例外
   */
  public void setState_INVALID_HEAVILY() throws Exception {

        getKaigoWelfareFacilityHeavilyCorrespondenceAddRadio().setEnabled(false);
