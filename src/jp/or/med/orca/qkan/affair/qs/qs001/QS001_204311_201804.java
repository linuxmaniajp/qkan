 ...
 * サブシステム 予定管理 (S)
 * プロセス サービス予定 (001)
 * プログラム サービスパターン介護予防短期入所療養介護（介護医療院）(QS001_204211_201804)
 *
 *****************************************************************
 */
...
/**
 * サービスパターン介護予防短期入所療養介護（介護医療院）(QS001_204311_201804)
 */
@SuppressWarnings("serial")
public class QS001_204311_201804 extends QS001_204311_201804Event {
    /**
     * コンストラクタです。
     */
    public QS001_204311_201804() {
        // 必須の事業所設定項目
        putCheckProviderBindPath("介護職員処遇改善加算", "4");

        // 値を転記する事業所設定項目
        putImportProviderBindPath("施設区分", "20430101", "20430101");
        putImportProviderBindPath("人員配置区分（I型）", "20430102", "20430102");
        putImportProviderBindPath("人員配置区分（II型）", "20430103", "20430103");
        putImportProviderBindPath("人員配置区分（特別型）", "20430104", "20430104");
        
        putImportProviderBindPath("ユニットケア体制", "20430107", "20430109");        
        putImportProviderBindPath("療養環境基準（廊下）", "20430108", "20430111");
        putImportProviderBindPath("療養環境基準（療養室）", "20430109", "20430112");  
        
        putImportProviderBindPath("サービス提供体制強化加算", "20430115", "20430122");
        
        


        // 値により入力を禁止する事業所設定項目
        putRestrictProviderBindPath("療養食", "20430112", "20430118");
        putRestrictProviderBindPath("若年性認知症利用者受入加算", "20430110", "20430117");
        putRestrictProviderBindPath("ユニットケア体制", "20430107", "20430109");
    }
    ...
    protected void shortStayHospitalRecuperationSpecificConsultationFeeActionPerformed(
        ...
        // 特別診療費ボタンのデータを引数に生成した特別診療費編集画面(QS001028)のshoModalを実行する。
    ...
    public void initialize() throws Exception {
        ...
        getShortStayHospitalRecuperationSpecificConsultationFee()
                .clearDataModel();
        // QS001S03_201804(特別診療費)よりサービス種類(SYSTEM_SERVICE_KIND_DETAIL)に応じたbindPath配列を取得し、特別診療費ボタンに設定する。
        getShortStayHospitalRecuperationSpecificConsultationFee()
                .setBindPathes(
                		QS001S03_201804
        ...
        comboItemMap.setData("20430123",
                QkanCommon.getArrayFromMasterCode(191, "20430123"));

        // ※時間区分
        // コードマスタデータよりCODE_ID：273（時間区分）を取得する。
        // 取得した値を、comboItemMapの KEY : 20430108 の VALUE として設定する。
        comboItemMap.setData("20430108",
                QkanCommon.getArrayFromMasterCode(273, "20430108"));
        // ※展開
        ...
        // ※設定
        // 食事提供
        VRBindPathParser.set("20430123", defaultMap, new Integer(1));
    ...
    public void providerSelected(VRMap provider) throws Exception {
        ...
        if (provider != null) {
            ...
            // 人員減算
            obj = VRBindPathParser.get("20430106", provider);
            if (obj != null) {
                // 事業所登録の減算項目に合わせ初期値を調整
                switch (ACCastUtilities.toInt(obj)) {
                case 1: // なしだった場合
                    VRBindPathParser.set("20430114", defaultMap, new Integer(1));
                    break;
                case 2: // 医師だった場合
                case 3: // 薬剤師だった場合
                case 4: // 看護職員だった場合
                case 5: // 介護職員だった場合
                    VRBindPathParser.set("20430114", defaultMap, new Integer(3));
                    break;
                }
            }

            // 夜間勤務条件基準
            obj = VRBindPathParser.get("20430105", provider);
            if (obj != null) {
                // 事業所登録の減算項目に合わせ初期値を調整
                switch (ACCastUtilities.toInt(obj)) {
                case 1: // 基準型
                    VRBindPathParser.set("20430113", defaultMap, new Integer(1));
                    break;
                case 2: // 加算型I
                    VRBindPathParser.set("20430110", defaultMap, new Integer(2));
                    break;
                case 3: // 加算型II
                    VRBindPathParser.set("20430110", defaultMap, new Integer(3));
                    break;
                case 4: // 加算型III
                    VRBindPathParser.set("20430110", defaultMap, new Integer(4));
                    break;
                case 5: // 加算型IV
                    VRBindPathParser.set("20430110", defaultMap, new Integer(5));
                    break;
                case 6: // 減算型
                    VRBindPathParser.set("20430113", defaultMap, new Integer(2));
                    break;
                }
            }   
        ...
        if (getSelectedProvider() != null) {
            ...
            case 1:
                ...
                return String.valueOf(ACCastUtilities.toInt(
                        VRBindPathParser.get("20430116", getSelectedProvider()),
                        0));
            case 2:
                // 食事時期が「朝のみ」の場合
                // 「朝」の食費を返す。
                return String.valueOf(ACCastUtilities.toInt(
                        VRBindPathParser.get("20430117", getSelectedProvider()),
                        0));
            case 3:
                // 食事時期が「昼のみ」の場合
                // 「昼」の食費を返す。
                return String.valueOf(ACCastUtilities.toInt(
                        VRBindPathParser.get("20430118", getSelectedProvider()),
                        0));
            case 4:
                // 食事時期が「夜のみ」の場合
                // 「夜」の食費を返す。
                return String.valueOf(ACCastUtilities.toInt(
                        VRBindPathParser.get("20430119", getSelectedProvider()),
                        0));
            case 5:
                // 食事時期が「朝昼」の場合
                ...
                return String.valueOf(ACCastUtilities.toInt(
                        VRBindPathParser.get("20430117", getSelectedProvider()),
                        0)
                        + ACCastUtilities.toInt(VRBindPathParser.get("20430118",
            ...
            case 6:
                ...
                return String.valueOf(ACCastUtilities.toInt(
                        VRBindPathParser.get("20430118", getSelectedProvider()),
                        0)
                        + ACCastUtilities.toInt(VRBindPathParser.get("20430119",
                                getSelectedProvider()), 0));
            case 7:
                // 食事時期が「夜朝」の場合
                ...
                return String.valueOf(ACCastUtilities.toInt(
                        VRBindPathParser.get("20430119", getSelectedProvider()),
                        0)
                        + ACCastUtilities.toInt(VRBindPathParser.get("20430117",
                                getSelectedProvider()), 0));
            }
        }
        // 空文字を返す。
