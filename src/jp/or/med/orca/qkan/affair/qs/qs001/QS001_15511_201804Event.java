 ...
 * アプリ: QKANCHO
 * 開発者: 亀井　陽一郎
 * 作成日: 2018/02/15  日本コンピューター株式会社 亀井　陽一郎 新規作成
 * 更新日: ----/--/--
 * システム 給付管理台帳 (Q)
 * サブシステム 予定管理 (S)
 * プロセス サービス予定 (001)
 * プログラム サービスパターン介護療養型医療施設（病院） (QS001_15511_201804)
 *
 *****************************************************************
 */
...
/**
 * サービスパターン介護療養型医療施設（病院）イベント定義(QS001_15511_201804) 
 */
public abstract class QS001_15511_201804Event extends QS001_15511_201804State implements QS001Service {
  /**
   * コンストラクタです。
   */
  public QS001_15511_201804Event(){
    addEvents();
  }
  /**
   * イベント発生条件を定義します。
  ...
  protected void addEvents() {
    ...
    getMedicalFacilityHospitalSubstraction().addListSelectionListener(new ListSelectionListener(){
        private boolean lockFlag = false;
        public void valueChanged(ListSelectionEvent e) {
            ...
            lockFlag = true;
            try {
                medicalFacilityHospitalSubstractionSelectionChanged(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
    ...
    getMedicalFacilityHospitalNightShiftSubtractionRadio().addListSelectionListener(new ListSelectionListener(){
        private boolean lockFlag = false;
        public void valueChanged(ListSelectionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                medicalFacilityHospitalNightShiftSubtractionRadioSelectionChanged(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void medicalFacilityHospitalSubstractionSelectionChanged(ListSelectionEvent e) throws Exception;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void medicalFacilityHospitalNightShiftSubtractionRadioSelectionChanged(ListSelectionEvent e) throws Exception;

  //変数定義

  public static final int SYSTEM_SERVICE_KIND_DETAIL = 15511;
