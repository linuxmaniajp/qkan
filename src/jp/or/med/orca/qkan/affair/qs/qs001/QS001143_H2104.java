...
/**
 * サービスパターン介護予防特定施設入居者生活介護(QS001143_H2104) 
 */
public class QS001143_H2104 extends QS001143_H2104Event {
  /**
   * コンストラクタです。
   */
  public QS001143_H2104(){
  }
   ...
   */
  protected void outsideServiceRadioActionPerformed(ActionEvent e) throws Exception{
    // 区分選択処理
    // 　画面状態制御
    checkState();
   ...
   * @throws Exception 処理例外
   */
  protected void facilitiesDivisionRadioActionPerformed(ActionEvent e) throws Exception{
    // 施設区分選択処理
    // 　画面状態制御
    checkState();
  ...
  public static void main(String[] args) {
    ...
    ACFrame.debugStart(new ACAffairInfo(QS001143_H2104.class.getName(), param));
  }
  ...
  public void providerSelected(VRMap provider) throws Exception{
        ...
        if (providerInfoMap != null) {
            ...
            //医療機関連携加算（事業所パネル）KEY：1350105の値をチェックする。
            switch(ACCastUtilities.toInt(VRBindPathParser.get("1350105", provider), 0)){
            case 1:
                // 値が1（なし）だった場合
                // defaultMapに KEY：1350127 VALUE：1（なし）を設定する。
                VRBindPathParser.set("1350127", defaultMap, new Integer(1));
                break;
            case 2:
                // 値が2（あり）だった場合
                // defaultMapに KEY：1350127 VALUE：2（あり）を設定する。
                VRBindPathParser.set("1350127", defaultMap, new Integer(2));
                break;
            }
            //障害者等支援加算（事業所パネル）KEY：1350106の値をチェックする。
            switch(ACCastUtilities.toInt(VRBindPathParser.get("1350106", provider), 0)){
            case 1:
                // 値が1（なし）だった場合
                // defaultMapに KEY：13501278 VALUE：1（なし）を設定する。
                VRBindPathParser.set("1350102", defaultMap, new Integer(1));
                break;
            case 2:
                // 値が2（あり）だった場合
                // defaultMapに KEY：13501278 VALUE：2（あり）を設定する。
                VRBindPathParser.set("1350102", defaultMap, new Integer(2));
                break;
            }
