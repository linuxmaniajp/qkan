...
/**
 * サービスパターン地域密着型通所介護(QS001_17811_201804)
 */
@SuppressWarnings("serial")
public class QS001_17811_201804 extends QS001_17811_201804Event {
    /**
     * コンストラクタです。
     */
    public QS001_17811_201804() {
        // 必須の事業所設定項目
        ...
        putImportProviderBindPath("生活相談員配置等加算", "1780118", "1780115"); //[H30.4改正]
        ...
        putRestrictProviderBindPath("ADL維持等加算", "1780122", "1780117"); //[H30.4改正]
        putRestrictProviderBindPath("生活相談員配置等加算", "1780118", "1780115"); //[H30.4改正]
        putRestrictProviderBindPath("生活機能向上連携加算", "1780119", "1780116"); //[H30.4改正]
    }
    ...
    protected void tsuusyoKaigoTimeContenaFacilityDivisionActionPerformed(
    	...
    	// 2018/03/03 [H30.4改正対応][Shinobu Hitaka] add - begin
    	// 地域密着型の場合は個別機能訓練体制選択状態設定
    	if (getTsuusyoKaigoTimeContenaFacilityDivision().getSelectedIndex() == 1){
            VRMap provider = getSelectedProvider();
            if (provider != null) {
            	setFunctionTraining(provider);
            }
    	}
    	// 2018/03/03 [H30.4改正対応][Shinobu Hitaka] add - end
    ...
    public static void main(String[] args) {
        ...
        ACFrame.debugStart(new ACAffairInfo(QS001_17811_201804.class.getName(),
                param));
    }
...
//            }
			// 個別機能訓練体制I
			switch (ACCastUtilities.toInt(VRBindPathParser.get("1780106", provider), 1)) {
			case 2: // I型
				VRBindPathParser.set("1780106", defaultMap, new Integer(2));
				break;
			default:
				VRBindPathParser.set("1780106", defaultMap, new Integer(1));
			}
			// 個別機能訓練体制II
			switch (ACCastUtilities.toInt(VRBindPathParser.get("1780120", provider), 1)) {
			case 2: // II型
				VRBindPathParser.set("1780107", defaultMap, new Integer(2));
				break;
			default:
				VRBindPathParser.set("1780107", defaultMap, new Integer(1));
			}
            // 個別機能訓練体制選択状態設定
			setFunctionTraining(provider);
            ...
            obj = VRBindPathParser.get("1780117", provider);
            if (obj != null) {
            ...
            obj = VRBindPathParser.get("1780116", provider);
            if (obj != null) {
                // 事業所登録の設定項目に合わせ初期値を調整
                switch (ACCastUtilities.toInt(obj)) {
                case 2: // ありだった場合
            ...
            // 共生型サービスの提供（生活介護事業所）
            obj = VRBindPathParser.get("1780114", provider);
            if (obj != null) {
                // 事業所登録の設定項目に合わせ初期値を調整
                switch (ACCastUtilities.toInt(obj)) {
                case 2: // ありだった場合
            ...
            // 共生型サービスの提供（自立訓練事業所）
            obj = VRBindPathParser.get("1780115", provider);
            if (obj != null) {
                // 事業所登録の設定項目に合わせ初期値を調整
                switch (ACCastUtilities.toInt(obj)) {
                case 2: // ありだった場合
    ...
    public void setFunctionTraining(VRMap provider) throws Exception {
		...
		// 個別機能訓練体制I
        int kinou1 = ACCastUtilities.toInt(VRBindPathParser.get("1780106", provider), 1);
		switch (kinou1) {
		case 2: // I型
		...
		// 個別機能訓練体制II
        int kinou2 = ACCastUtilities.toInt(VRBindPathParser.get("1780120", provider), 1);
		switch (kinou2) {
		case 2: // II型
