...
/**
 * サービスパターン介護老人保健施設イベント定義(QS001119_H2104) 
 */
public abstract class QS001119_H2104Event extends QS001119_H2104State implements QS001Service {
  /**
   * コンストラクタです。
   */
  public QS001119_H2104Event(){
    addEvents();
  }
  /**
   * イベント発生条件を定義します。
  ...
  protected void addEvents() {
    ...
    getKaigoHealthCareOfTheAgedDivision().addListSelectionListener(new ListSelectionListener(){
        private boolean lockFlag = false;
        public void valueChanged(ListSelectionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                kaigoHealthCareOfTheAgedDivisionSelectionChanged(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void kaigoHealthCareOfTheAgedDivisionSelectionChanged(ListSelectionEvent e) throws Exception;
