...
/**
 * サービスパターンその他の生活支援サービス(その他定額)(QS001_51411_201804)
 */
public class QS001_51411_201804 extends QS001_51411_201804Event {
	/**
	 * コンストラクタです。
	 */
	public QS001_51411_201804() {
	}
	...
	public static void main(String[] args) {
		...
		ACFrame.debugStart(new ACAffairInfo(QS001_51411_201804.class.getName(),
				param));
	}
	...
	public VRMap getValidData() throws Exception {
		...
		if ((!getSoAEBeginTime().isValidDate())
				|| (!getSoAEEndTime().isValidDate())) {
		...
		// ※返却用レコードから不要なキーを除去
		if (ACTextUtilities.isNullText(getSoAEBeginTime().getText())) {
			// 開始時刻コンボが空欄の場合
			// 返却用レコードから、開始時刻コンボのbindPathを除去する。
			data.remove(getSoAEBeginTime().getBindPath());
		}
		if (ACTextUtilities.isNullText(getSoAEEndTime().getText())) {
			// 終了時刻コンボが空欄の場合
			// 返却用レコードから、終了時刻コンボのbindPathを除去する。
			data.remove(getSoAEEndTime().getBindPath());
		}
		// サービス一覧のデータを追加
