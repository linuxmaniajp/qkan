...
/**
 * サービスパターン介護療養型医療施設（病院）(QS001120_H2104)
 */
public class QS001120_H2104 extends QS001120_H2104Event {
    /**
     * コンストラクタです。
     */
    public QS001120_H2104() {
    }
     ...
     * @throws Exception 処理例外
     */
    protected void oralMaintenanceRadioSelectionChanged(ListSelectionEvent e)
            throws Exception {
    ...
    public static void main(String[] args) {
        ...
        ACFrame.debugStart(new ACAffairInfo(QS001120_H2104.class.getName(),
                param));
    }
    ...
    public void providerSelected(VRMap provider) throws Exception {
        ...
        if (provider != null) {
            ...
            Object obj;
            // 人員配置区分
            obj = VRBindPathParser.get("1530129", provider);
            if (obj != null) {
                defaultMap.setData("1530102", obj);
            }
            // 施設区分
            obj = VRBindPathParser.get("1530128", provider);
            if (obj != null) {
                defaultMap.setData("1530162", obj);
            }
            // 夜間勤務条件基準
            obj = VRBindPathParser.get("1530130", provider);
            if (obj != null) {
                defaultMap.setData("1530163", obj);
            }
//            // 療養環境基準
//            obj = VRBindPathParser.get("1530127", provider);
//            if (obj != null) {
//                defaultMap.setData("1530168", obj);
//            }
            // 医師の配置基準
            ...
            // 若年性認知症患者受入加算
            obj = VRBindPathParser.get("1530121", provider);
            ...
            //常にdefaultMapに KEY：1530164 VALUE：1（なし）を設定する。
            VRBindPathParser.set("1530164", defaultMap, new Integer(1));

            //認知症専門ケア加算（事業所パネル）KEY：1530123の値をチェックする。
            obj = VRBindPathParser.get("1530123", provider);
            switch (ACCastUtilities.toInt(obj,0)) {
            //値が次のいずれかだった場合
            case 2:
                //2（加算I）
            ...
            //常にdefaultMapに KEY：1530166 VALUE：1（なし）を設定する。
            VRBindPathParser.set("1530166", defaultMap, new Integer(1));   
            
            
            // サービス提供体制強化加算
            obj = VRBindPathParser.get("1530124", provider);
            if (obj != null) {
                defaultMap.setData("1530167", obj);
            }
            

            // 栄養マネジメント加算
            obj = VRBindPathParser.get("1530126", provider);
            if (obj != null) {
            ...
            // 療養食加算
            obj = VRBindPathParser.get("1530122", provider);
            if (obj != null) {
                switch (ACCastUtilities.toInt(obj)) {
                case 1:
                    // 体制なしだった場合
...
//                    defaultMap.setData("1530114", obj);
            ...
            //常にdefaultMapに KEY：1530114 VALUE：1（なし）を設定する。
            VRBindPathParser.set("1530114", defaultMap, new Integer(1));   
            // [ID:0000481][Tozo TANAKA] 2009/04/08 add end

            // ユニットケアの整備
            obj = VRBindPathParser.get("1530131", provider);
            if (obj != null) {
                switch (ACCastUtilities.toInt(obj)) {
                case 1:
                    // 体制なしだった場合
