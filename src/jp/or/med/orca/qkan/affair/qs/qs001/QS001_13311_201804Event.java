...
/**
 * サービスパターン特定施設入所者生活介護イベント定義(QS001_13311_201804) 
 */
public abstract class QS001_13311_201804Event extends QS001_13311_201804State implements QS001Service {
  /**
   * コンストラクタです。
   */
  public QS001_13311_201804Event(){
    addEvents();
  }
  /**
   * イベント発生条件を定義します。
  ...
  protected void addEvents() {
    ...
    getVisitCareRadio().addListSelectionListener(new ListSelectionListener(){
        private boolean lockFlag = false;
        public void valueChanged(ListSelectionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                visitCareRadioSelectionChanged(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
    ...
    getTerminalCareAddRadioGroup().addListSelectionListener(new ListSelectionListener(){
        private boolean lockFlag = false;
        public void valueChanged(ListSelectionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                terminalCareAddRadioGroupSelectionChanged(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
    ...
    getCalculationDivisionRadio().addListSelectionListener(new ListSelectionListener(){
        private boolean lockFlag = false;
        public void valueChanged(ListSelectionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                calculationDivisionRadioSelectionChanged(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
  ...
  /**
   * 「訪問介護提供サービス区分選択」イベントです。
   * @param e イベント情報
   * @throws Exception 処理例外
   */
  protected abstract void visitCareRadioSelectionChanged(ListSelectionEvent e) throws Exception;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void terminalCareAddRadioGroupSelectionChanged(ListSelectionEvent e) throws Exception;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void calculationDivisionRadioSelectionChanged(ListSelectionEvent e) throws Exception;
