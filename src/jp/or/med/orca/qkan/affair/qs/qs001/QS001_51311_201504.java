...
/**
 * サービスパターンその他の生活支援サービス(その他定率)(QS001_51311_201504)
 */
public class QS001_51311_201504 extends QS001_51311_201504Event {
	/**
	 * コンストラクタです。
	 */
	public QS001_51311_201504() {
	}
	...
	public static void main(String[] args) {
		...
		ACFrame.debugStart(new ACAffairInfo(QS001_51311_201504.class.getName(),
				param));
	}
	...
	public VRMap getValidData() throws Exception {
		...
		if ((!getSoADBeginTime().isValidDate())
				|| (!getSoADEndTime().isValidDate())) {
		...
		// ※返却用レコードから不要なキーを除去
		if (ACTextUtilities.isNullText(getSoADBeginTime().getText())) {
			// 開始時刻コンボが空欄の場合
			// 返却用レコードから、開始時刻コンボのbindPathを除去する。
			data.remove(getSoADBeginTime().getBindPath());
		}
		if (ACTextUtilities.isNullText(getSoADEndTime().getText())) {
			// 終了時刻コンボが空欄の場合
			// 返却用レコードから、終了時刻コンボのbindPathを除去する。
			data.remove(getSoADEndTime().getBindPath());
		}
		// サービス一覧のデータを追加
