 ...
 * サブシステム 予定管理 (S)
 * プロセス サービス予定 (001)
 * プログラム サービスパターン介護予防短期入所療養介護（診療所） (QS001140_H2004)
 *
 *****************************************************************
 */
...
/**
 * サービスパターン介護予防短期入所療養介護（診療所）(QS001140_H2004) 
 */
public class QS001140_H2004 extends QS001140_H2004Event {
  /**
   * コンストラクタです。
   */
  public QS001140_H2004(){
  }
  ...
  public static void main(String[] args) {
      ...
      ACFrame.debugStart(new ACAffairInfo(QS001140.class.getName(), param));
  }
  ...
  public void providerSelected(VRMap provider) throws Exception {
      ...
      if (provider != null) {
          ...
          // 療養環境基準（事業所パネル）KEY：1260204の値をチェックする。
          switch (ACCastUtilities.toInt(VRBindPathParser.get("1260204",
                  provider), 0)) {
          case 1:
              // 値が1（基準型）だった場合
              // defaultMapに KEY：1260205 VALUE：1（基準型）を設定する。
              VRBindPathParser.set("1260205", defaultMap, new Integer(1));
              break;
          case 0:
          case 2:
              // 値が0（未選択）もしくは、値が2（減算型Ⅰ）だった場合
              // defaultMapに KEY：1260205 VALUE：0（未設定）を設定する。
        	  // 過去の体制からの値なので値を振り替える
              VRBindPathParser.set("1260205", defaultMap, new Integer(0));
              break;
          case 3:
              // 値が3（減算型Ⅱ）だった場合
              // defaultMapに KEY：1260205 VALUE：3（減算型Ⅱ）を設定する。
              VRBindPathParser.set("1260205", defaultMap, new Integer(3));
              break;
          }
          ...
          // 栄養管理の評価（事業所パネル）KEY：1260206の値をチェックする。
          switch (ACCastUtilities.toInt(VRBindPathParser.get("1260206",
                  provider), 0)) {
          case 1:
              // 値が1（なし）だった場合
              // defaultMapに KEY：1260206 VALUE：1（なし）を設定する。
              VRBindPathParser.set("1260206", defaultMap, new Integer(1));
              break;
          case 2:
              // 値が2（栄養士）だった場合
              // defaultMapに KEY：1260206 VALUE：3（栄養士）を設定する。
              VRBindPathParser.set("1260206", defaultMap, new Integer(3));
              break;
          case 3:
              // 値が3（管理栄養士）だった場合
              // defaultMapに KEY：1260206 VALUE：2（管理栄養士）を設定する。
              VRBindPathParser.set("1260206", defaultMap, new Integer(2));
              break;
          }
          // ※展開
          // 自身(this)にdefaultMapに設定する。
  ...
  public VRMap getValidData() throws Exception {
      ...
      // 設備基準減算に値が設定されていない場合
      if(getShortStayClinicRecuperationProvisionBaseType().getSelectedIndex() < 1){
          QkanMessageList.getInstance().QS001_ERROR_OF_NO_CONTENT();
          return null;          
      }
