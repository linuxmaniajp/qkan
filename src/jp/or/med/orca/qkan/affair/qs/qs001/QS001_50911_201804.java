...
/**
 * サービスパターンその他の生活支援サービス(配食定率)(QS001_50911_201804)
 */
public class QS001_50911_201804 extends QS001_50911_201804Event {
	/**
	 * コンストラクタです。
	 */
	public QS001_50911_201804() {
	}
	...
	public static void main(String[] args) {
		...
		ACFrame.debugStart(new ACAffairInfo(QS001_50911_201804.class.getName(),
				param));
	}
	...
	public VRMap getValidData() throws Exception {
		...
		if ((!getSoA9BeginTime().isValidDate())
				|| (!getSoA9EndTime().isValidDate())) {
		...
		// ※返却用レコードから不要なキーを除去
		if (ACTextUtilities.isNullText(getSoA9BeginTime().getText())) {
			// 開始時刻コンボが空欄の場合
			// 返却用レコードから、開始時刻コンボのbindPathを除去する。
			data.remove(getSoA9BeginTime().getBindPath());
		}
		if (ACTextUtilities.isNullText(getSoA9EndTime().getText())) {
			// 終了時刻コンボが空欄の場合
			// 返却用レコードから、終了時刻コンボのbindPathを除去する。
			data.remove(getSoA9EndTime().getBindPath());
		}
		// サービス一覧のデータを追加
