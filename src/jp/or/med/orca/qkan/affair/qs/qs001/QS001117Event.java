 ...
 * アプリ: QKANCHO
 * 開発者: 堤 瑞樹
 * 作成日: 2006/03/15  日本コンピューター株式会社 堤 瑞樹 新規作成
 * 更新日: ----/--/--
 * システム 給付管理台帳 (Q)
 * サブシステム 予定管理 (S)
 * プロセス サービス予定 (001)
...
/**
 * サービスパターン居宅介護支援イベント定義(QS001117) 
 */
public abstract class QS001117Event extends QS001117SQL implements QS001Service {
  /**
   * コンストラクタです。
   */
  public QS001117Event(){
    addEvents();
  }
  /**
   * イベント発生条件を定義します。
  ...
  protected void addEvents() {
    ...
    getKaigoSupportSpecificProviderSubtractionRadio().addActionListener(new ActionListener(){
        private boolean lockFlag = false;
        public void actionPerformed(ActionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                kaigoSupportSpecificProviderSubtractionRadioActionPerformed(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
    ...
    getKaigoSupportSpecificStandardRadio().addActionListener(new ActionListener(){
        private boolean lockFlag = false;
        public void actionPerformed(ActionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                kaigoSupportSpecificStandardRadioActionPerformed(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void kaigoSupportSpecificProviderSubtractionRadioActionPerformed(ActionEvent e) throws Exception;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void kaigoSupportSpecificStandardRadioActionPerformed(ActionEvent e) throws Exception;
