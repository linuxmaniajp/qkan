...
/**
 * サービスパターン介護療養型医療施設（診療所）(QS001121_H2004) 
 */
public class QS001121_H2004 extends QS001121_H2004Event {
  /**
   * コンストラクタです。
   */
  public QS001121_H2004(){
  }
  ...
  public void providerSelected(VRMap provider) throws Exception {
      ...
      if (provider != null) {
          ...
          // 療養環境基準（事業所パネル）KEY：1530203の値をチェックする。
          switch (ACCastUtilities.toInt(VRBindPathParser.get("1530203",
                  provider), 0)) {
          case 1:
              // 値が1（基準型）だった場合
              // defaultMapに KEY：1530205 VALUE：1（基準型）を設定する。
              VRBindPathParser.set("1530205", defaultMap, new Integer(1));
              break;
          case 0:
          case 2:
              // 値が0（未選択）もしくは、値が2（減算型Ⅰ）だった場合
              // defaultMapに KEY：1530205 VALUE：0（基準型）を設定する。
        	  // 過去の体制からの値なので値を振り替える
              VRBindPathParser.set("1530205", defaultMap, new Integer(0));
              break;
          case 3:
              // 値が3（減算型Ⅱ）だった場合
              // defaultMapに KEY：1530205 VALUE：3（減算型Ⅱ）を設定する。
              VRBindPathParser.set("1530205", defaultMap, new Integer(3));
              break;
          }
          ...
          // 設備環境減算
          switch (ACCastUtilities.toInt(VRBindPathParser.get("1530219",
                  provider), 0)) {
          case 0:
              // 値が0（未設定）だった場合
              // defaultMapに KEY：1530260 VALUE：0（未選択）を設定する。
              VRBindPathParser.set("1530260", defaultMap, new Integer(0));
              break;
          case 1:
              // 値が1（基準型）だった場合
              // defaultMapに KEY：1530260 VALUE：1（基準型）を設定する。
              VRBindPathParser.set("1530260", defaultMap, new Integer(1));
              break;
          case 2:
              // 値が2（減算型）だった場合
              // defaultMapに KEY：1530260 VALUE：2（減算型）を設定する。
              VRBindPathParser.set("1530260", defaultMap, new Integer(2));
              break;
          }
  ...
  public VRMap getValidData() throws Exception {
      ...
      // 療養環境減算が設定されていない場合
      if(getMedicalFacilityClinicEnvironmentalRadio().getSelectedIndex() <1){
          // 内容詳細に不備がある場合
          QkanMessageList.getInstance().QS001_ERROR_OF_NO_CONTENT();
          return null;
      }
      // 設備基準減算が設定されていない場合
      if(getMedicalFacilityClinicProvisionBaseRadio().getSelectedIndex() < 1){
          // 内容詳細に不備がある場合
          QkanMessageList.getInstance().QS001_ERROR_OF_NO_CONTENT();
          return null;          
      }
