...
/**
 * サービスパターンその他の生活支援サービス(見守り定率)(QS001_51111_201504)
 */
public class QS001_51111_201504 extends QS001_51111_201504Event {
	/**
	 * コンストラクタです。
	 */
	public QS001_51111_201504() {
	}
	...
	public static void main(String[] args) {
		...
		ACFrame.debugStart(new ACAffairInfo(QS001_51111_201504.class.getName(),
				param));
	}
	...
	public VRMap getValidData() throws Exception {
		...
		if ((!getSoABBeginTime().isValidDate())
				|| (!getSoABEndTime().isValidDate())) {
		...
		// ※返却用レコードから不要なキーを除去
		if (ACTextUtilities.isNullText(getSoABBeginTime().getText())) {
			// 開始時刻コンボが空欄の場合
			// 返却用レコードから、開始時刻コンボのbindPathを除去する。
			data.remove(getSoABBeginTime().getBindPath());
		}
		if (ACTextUtilities.isNullText(getSoABEndTime().getText())) {
			// 終了時刻コンボが空欄の場合
			// 返却用レコードから、終了時刻コンボのbindPathを除去する。
			data.remove(getSoABEndTime().getBindPath());
		}
		// サービス一覧のデータを追加
