...
/**
 * サービスパターンその他の生活支援サービス(配食定額)(QS001_51011_201504)
 */
public class QS001_51011_201504 extends QS001_51011_201504Event {
	/**
	 * コンストラクタです。
	 */
	public QS001_51011_201504() {
	}
	...
	public static void main(String[] args) {
		...
		ACFrame.debugStart(new ACAffairInfo(QS001_51011_201504.class.getName(),
				param));
	}
	...
	public VRMap getValidData() throws Exception {
		...
		if ((!getSoAABeginTime().isValidDate())
				|| (!getSoAAEndTime().isValidDate())) {
		...
		// ※返却用レコードから不要なキーを除去
		if (ACTextUtilities.isNullText(getSoAABeginTime().getText())) {
			// 開始時刻コンボが空欄の場合
			// 返却用レコードから、開始時刻コンボのbindPathを除去する。
			data.remove(getSoAABeginTime().getBindPath());
		}
		if (ACTextUtilities.isNullText(getSoAAEndTime().getText())) {
			// 終了時刻コンボが空欄の場合
			// 返却用レコードから、終了時刻コンボのbindPathを除去する。
			data.remove(getSoAAEndTime().getBindPath());
		}
		// サービス一覧のデータを追加
