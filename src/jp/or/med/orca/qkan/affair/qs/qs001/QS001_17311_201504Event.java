 ...
 * サブシステム 予定管理 (S)
 * プロセス サービス予定 (001)
 * プログラム サービスパターン小規模多機能型介護 (QS001_17311_201504)
 *
 *****************************************************************
 */
...
/**
 * サービスパターン小規模多機能型介護イベント定義(QS001_17311_201504) 
 */
public abstract class QS001_17311_201504Event extends QS001_17311_201504SQL implements QS001Service {
  /**
   * コンストラクタです。
   */
  public QS001_17311_201504Event(){
    addEvents();
  }
  /**
   * イベント発生条件を定義します。
  ...
  protected void addEvents() {
    ...
    getNurseStaffDispositionAddRadioGroup().addActionListener(new ActionListener(){
        private boolean lockFlag = false;
        public void actionPerformed(ActionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                nurseStaffDispositionAddRadioGroupActionPerformed(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
    ...
    getTerminalCareInformationRadioGroup().addListSelectionListener(new ListSelectionListener(){
        private boolean lockFlag = false;
        public void valueChanged(ListSelectionEvent e) {
            ...
            lockFlag = true;
            try {
                terminalCareInformationRadioGroupSelectionChanged(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
    ...
    getCalculationDivision().addListSelectionListener(new ListSelectionListener(){
        private boolean lockFlag = false;
        public void valueChanged(ListSelectionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                calculationDivisionSelectionChanged(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void nurseStaffDispositionAddRadioGroupActionPerformed(ActionEvent e) throws Exception;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void terminalCareInformationRadioGroupSelectionChanged(ListSelectionEvent e) throws Exception;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void calculationDivisionSelectionChanged(ListSelectionEvent e) throws Exception;
