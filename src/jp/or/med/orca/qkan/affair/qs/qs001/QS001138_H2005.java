 ...
 * サブシステム 予定管理 (S)
 * プロセス サービス予定 (001)
 * プログラム サービスパターン介護予防短期入所療養介護（介護老人保健施設） (QS001138_H2005)
 *
 *****************************************************************
 */
...
/**
 * サービスパターン介護予防短期入所療養介護（介護老人保健施設）(QS001138_H2005) 
 */
public class QS001138_H2005 extends QS001138_H2005Event {
  /**
   * コンストラクタです。
   */
  public QS001138_H2005(){
  }
  ...
  public static void main(String[] args) {
      ...
      ACFrame.debugStart(new ACAffairInfo(QS001138.class.getName(), param));
  }
  ...
  public void providerSelected(VRMap provider) throws Exception {
      ...
      if (provider != null) {
          ...
          // 施設等の区分（体制）
          index = ACCastUtilities.toInt(VRBindPathParser.get("1250116", provider),0);
          // 設定
          ...
          // 職員の欠員による減算の状況（事業所パネル）KEY：1250103の値をチェックする。
          switch (ACCastUtilities.toInt(VRBindPathParser.get("1250103",
                  provider), 0)) {
          case 1:
              // 値が1（なし）だった場合
          ...
          // 栄養管理の評価（事業所パネル）KEY：1250107の値をチェックする。
          switch (ACCastUtilities.toInt(VRBindPathParser.get("1250107",
                  provider), 0)) {
          case 1:
              // 値が1（なし）だった場合
              // defaultMapに KEY：1250107 VALUE：1（なし）を設定する。
              VRBindPathParser.set("1250107", defaultMap, new Integer(1));
              break;
          case 2:
              // 値が2（栄養士）だった場合
              // defaultMapに KEY：1250107 VALUE：3（栄養士）を設定する。
              VRBindPathParser.set("1250107", defaultMap, new Integer(3));
              break;
          case 3:
              // 値が3（管理栄養士）だった場合
              // defaultMapに KEY：1250107 VALUE：2（管理栄養士）を設定する。
              VRBindPathParser.set("1250107", defaultMap, new Integer(2));
              break;
          }
          
          // 療養体制維持特別加算
          index = ACCastUtilities.toInt(VRBindPathParser.get("1250117", provider),0);
          // 設定
  ...
  public void checkState() throws Exception {
      ...
      switch(getShortStayRecuperationHealthFacilityInstitutionDivisionRadio().getSelectedIndex()){
      case 1:
          setState_UNIT_CARE_ENABLE_FALSE();
          break;
      }
  ...
  public void checkMeatCost() throws Exception {
    ...
    public void binded() throws Exception{
        if (getSpecialMedicalExpense()
                .getDataModel().isEmpty()) {
            // 特別療養費ボタンのデータが空の場合
            // 特別療養費ラベルに「設定なし」と表示する。
            getSpecialMedicalExpenseLabel()
                    .setText("設定なし");
        } else {
            // 特別療養費ボタンのデータが空ではない場合
            // 特別療養費ラベルに「設定あり」と表示する。
