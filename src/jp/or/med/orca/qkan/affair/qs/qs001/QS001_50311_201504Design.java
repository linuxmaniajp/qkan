 ...
 * サブシステム 予定管理 (S)
 * プロセス サービス予定 (001)
 * プログラム サービスパターン訪問型サービス(独自定率) (QS001_50311_201504)
 *
 *****************************************************************
 */
...
/**
 * サービスパターン訪問型サービス(独自定率)画面項目デザイン(QS001_50311_201504) 
 */
public class QS001_50311_201504Design extends QS001ServicePanelSogo {
  //GUIコンポーネント

  private ACPanel soA3Patterns;

  private ACPanel soA3Detail;

  private ACBackLabelContainer soA3TimeContena;

  private ACTimeComboBox soA3BeginTime;

  private ACLabelContainer soA3BeginTimeContainer;

  private ACComboBoxModelAdapter soA3BeginTimeModel;

  private ACTimeComboBox soA3EndTime;

  private ACLabelContainer soA3EndTimeContainer;

  private ACComboBoxModelAdapter soA3EndTimeModel;
  ...
  private ACBackLabelContainer soA3ItemCodeContena;
   ...
   */
  public ACPanel getSoA3Patterns(){
    if(soA3Patterns==null){

      soA3Patterns = new ACPanel();

      soA3Patterns.setAutoWrap(false);
   ...
   * @return 内容パネル
   */
  public ACPanel getSoA3Detail(){
    if(soA3Detail==null){

      soA3Detail = new ACPanel();

      soA3Detail.setHgrid(200);
   ...
   * @return 提供時間コンテナ
   */
  public ACBackLabelContainer getSoA3TimeContena(){
    if(soA3TimeContena==null){

      soA3TimeContena = new ACBackLabelContainer();

      addSoA3TimeContena();
    }
    return soA3TimeContena;
   ...
   * @return 開始時刻コンボ
   */
  public ACTimeComboBox getSoA3BeginTime(){
    if(soA3BeginTime==null){

      soA3BeginTime = new ACTimeComboBox();

      getSoA3BeginTimeContainer().setText("開始時刻");

      soA3BeginTime.setBindPath("3");

      soA3BeginTime.setModelBindPath("3");

      soA3BeginTime.setRenderBindPath("CONTENT");

      soA3BeginTime.setModel(getSoA3BeginTimeModel());

      addSoA3BeginTime();
    }
    return soA3BeginTime;
   ...
   * @return 開始時刻コンボコンテナ
   */
  protected ACLabelContainer getSoA3BeginTimeContainer(){
    if(soA3BeginTimeContainer==null){
      soA3BeginTimeContainer = new ACLabelContainer();
      soA3BeginTimeContainer.setFollowChildEnabled(true);
      soA3BeginTimeContainer.setVAlignment(VRLayout.CENTER);
      soA3BeginTimeContainer.add(getSoA3BeginTime(), null);
    }
    return soA3BeginTimeContainer;
  }
   ...
   * @return 開始時刻コンボモデル
   */
  protected ACComboBoxModelAdapter getSoA3BeginTimeModel(){
    if(soA3BeginTimeModel==null){
      soA3BeginTimeModel = new ACComboBoxModelAdapter();
      addSoA3BeginTimeModel();
    }
    return soA3BeginTimeModel;
  }
   ...
   * @return 終了時刻コンボ
   */
  public ACTimeComboBox getSoA3EndTime(){
    if(soA3EndTime==null){

      soA3EndTime = new ACTimeComboBox();

      getSoA3EndTimeContainer().setText("終了時刻");

      soA3EndTime.setBindPath("4");

      soA3EndTime.setModelBindPath("4");

      soA3EndTime.setRenderBindPath("CONTENT");

      soA3EndTime.setModel(getSoA3EndTimeModel());
   ...
   * @return 終了時刻コンボコンテナ
   */
  protected ACLabelContainer getSoA3EndTimeContainer(){
    if(soA3EndTimeContainer==null){
      soA3EndTimeContainer = new ACLabelContainer();
      soA3EndTimeContainer.setFollowChildEnabled(true);
      soA3EndTimeContainer.setVAlignment(VRLayout.CENTER);
      soA3EndTimeContainer.add(getSoA3EndTime(), null);
    }
    return soA3EndTimeContainer;
  }
   ...
   * @return 終了時刻コンボモデル
   */
  protected ACComboBoxModelAdapter getSoA3EndTimeModel(){
    if(soA3EndTimeModel==null){
      soA3EndTimeModel = new ACComboBoxModelAdapter();
      addSoA3EndTimeModel();
    }
    return soA3EndTimeModel;
  }
   ...
   */
  public ACBackLabelContainer getSoA3ItemCodeContena(){
    if(soA3ItemCodeContena==null){

      soA3ItemCodeContena = new ACBackLabelContainer();

      addSoA3ItemCodeContena();
    }
    return soA3ItemCodeContena;
  ...
  public ACTextField getItemCodeText(){
    if(itemCodeText==null){
      ...
      getItemCodeTextContainer().setText("A3");
   ...
   * コンストラクタです。
   */
  public QS001_50311_201504Design() {
  ...
  protected void addThis(){
    ...
    this.add(getSoA3Patterns(), VRLayout.WEST);
   ...
   */
  protected void addSoA3Patterns(){

    soA3Patterns.add(getSoA3Detail(), VRLayout.NORTH);
   ...
   * 内容パネルに内部項目を追加します。
   */
  protected void addSoA3Detail(){

    soA3Detail.add(getSoA3TimeContena(), VRLayout.FLOW_DOUBLEINSETLINE_RETURN);

    soA3Detail.add(getInsurerNameContainer(), VRLayout.FLOW_INSETLINE);

    soA3Detail.add(getSoA3ItemCodeContena(), VRLayout.FLOW_DOUBLEINSETLINE_RETURN);

    soA3Detail.add(getCodeFindButton(), VRLayout.FLOW);

    soA3Detail.add(getDeleteCodeButton(), VRLayout.FLOW_RETURN);

    soA3Detail.add(getSvCodeTable(), VRLayout.FLOW_RETURN);
   ...
   * 提供時間コンテナに内部項目を追加します。
   */
  protected void addSoA3TimeContena(){

    soA3TimeContena.add(getSoA3BeginTimeContainer(), VRLayout.FLOW);

    soA3TimeContena.add(getSoA3EndTimeContainer(), VRLayout.FLOW);
   ...
   * 開始時刻コンボに内部項目を追加します。
   */
  protected void addSoA3BeginTime(){
   ...
   */
  protected void addSoA3BeginTimeModel(){
   ...
   */
  protected void addSoA3EndTimeModel(){
   ...
   */
  protected void addSoA3ItemCodeContena(){

    soA3ItemCodeContena.add(getItemCodeTextContainer(), VRLayout.FLOW);

    soA3ItemCodeContena.add(getAddCodeToListButton(), VRLayout.FLOW);
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      ACFrame.debugStart(new ACAffairInfo(QS001_50311_201504Design.class.getName()));
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
   ...
   * 自身を返します。
   */
  protected QS001_50311_201504Design getThis() {
    return this;
  }
}
