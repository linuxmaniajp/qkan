...
/**
 * サービスパターン介護療養型医療施設（診療所）(QS001_15312_201804)
 */
@SuppressWarnings("serial")
public class QS001_15312_201804 extends QS001_15312_201804Event {
    /**
     * コンストラクタです。
     */
    public QS001_15312_201804() {
        ...
        putImportProviderBindPath("人員配置区分（ユニット型病院療養型用）", "1530226", "1530218");
        putImportProviderBindPath("入院患者に関する基準", "1530227", "1530267"); //[H30.4改正]
     ...
     * @throws Exception 処理例外
     */
    protected void medicalFacilityClinicNutritionRadioSelectionChanged
    ...
    /**
     * 「入院患者に関する基準の変更」イベントです。
     * 
     * @param e イベント情報
     * @throws Exception 処理例外
     */
	@Override
	protected void hospitalizedPatientsCriteriaRadioSelectionChanged(
			ListSelectionEvent e) throws Exception {
		checkState();
	}

    /**
     * 「人員配置区分（病院療養型用）の変更」イベントです。
     * 
     * @param e イベント情報
     * @throws Exception 処理例外
     */
	@Override
	protected void staffAssignment1DivisionSelectionChanged(
			ListSelectionEvent e) throws Exception {
		checkState();
	}

    /**
     * 「人員配置区分（ユニット型病院療養型用）の変更」イベントです。
     * 
     * @param e イベント情報
     * @throws Exception 処理例外
     */
	@Override
	protected void staffAssignment2DivisionSelectionChanged(
			ListSelectionEvent e) throws Exception {
		checkState();
	}
    ...
    public static void main(String[] args) {
        ...
        ACFrame.debugStart(new ACAffairInfo(QS001_15312_201804.class.getName(),
                param));
    }
    ...
    public void checkState() throws Exception {
    	...
    	// 入院患者に関する基準が「減算型」の場合で無効となっている項目があれば有効にしておく
    	setState_VALID_HOSPITAL_PATIENT_RELATED();
    	// 2018/02/20 [H30.4改正対応][Yoichiro Kamei] add - end
    	
        // 栄養マネジメント加算の状態を基に経口移行加算、経口維持加算の有効状態を変更
        if (getMedicalFacilityClinicNutritionRadioItem2().isSelected()) {
            // 経口移行加算の状態を元に制御
            if (getMedicalFacilityClinicOralSwitchRadioItem2().isSelected()) {
        ...
        // 2018/02/20 [H30.4改正対応][Yoichiro Kamei] add - begin
        // 人員配置区分の選択により「入院患者に関する基準」の状態制御
        boolean enabled = true;
        // 人員配置区分（病院療養型用）が有効な場合
        if (getStaffAssignment1Division().isEnabled()
        		&& getStaffAssignment1Division().isVisible()) {
            // 人員配置区分をチェックする
            switch (getStaffAssignment1Division().getSelectedIndex()) {
            case 2: //I型（強化型A）
            case 3: //I型（強化型B）
            case 5: //II型（強化型）
            	enabled = false;
        ...
        // 人員配置区分（ユニット型病院療養型用）が有効な場合
        if (getStaffAssignment2Division().isEnabled()
        		&& getStaffAssignment2Division().isVisible()) {
            // 人員配置区分をチェックする
            switch (getStaffAssignment2Division().getSelectedIndex()) {
            case 2: //I型（強化型A）
            case 3: //I型（強化型B）
            	enabled = false;
            	break;
        ...
        if (enabled) {
        	// 入院患者基準を有効にします。
        	setState_VALID_HOSPITALIZED_PATIENTS_CRITERIA();
        } else {
        	// 入院患者基準を無効にします。
        	setState_INVALID_HOSPITALIZED_PATIENTS_CRITERIA();
        }
        
        // 入院患者に関する基準が「減算型」の場合
        if (getHospitalizedPatientsCriteriaRadioItem2().isSelected()
        		&& getHospitalizedPatientsCriteriaRadioItem2().isEnabled()) {
        	// 入院患者基準の関連加算を無効にします。
        	setState_INVALID_HOSPITAL_PATIENT_RELATED();
        }
        // 2018/02/20 [H30.4改正対応][Yoichiro Kamei] add - end
