 ...
 * アプリ: QKANCHO
 * 開発者: 樋口　雅彦
 * 作成日: 2012/03/23  日本コンピューター株式会社 樋口　雅彦 新規作成
 * 更新日: ----/--/--
 * システム 給付管理台帳 (Q)
 * サブシステム 予定管理 (S)
 * プロセス サービス予定 (001)
 * プログラム サービスパターン訪問看護（介護) (QS001_11311_201504)
 *
 *****************************************************************
 */
...
/**
 * サービスパターン訪問看護（介護)イベント定義(QS001_11311_201504) 
 */
public abstract class QS001_11311_201504Event extends QS001_11311_201504State implements QS001Service {
  /**
   * コンストラクタです。
   */
  public QS001_11311_201504Event(){
    addEvents();
  }
  /**
   * イベント発生条件を定義します。
  ...
  protected void addEvents() {
    ...
    getHoumonKangoKaigoTime().addListSelectionListener(new ListSelectionListener(){
        private boolean lockFlag = false;
        public void valueChanged(ListSelectionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                houmonKangoKaigoTimeSelectionChanged(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
    ...
    getSpecialManagementRadioGroup().addListSelectionListener(new ListSelectionListener(){
        private boolean lockFlag = false;
        public void valueChanged(ListSelectionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                specialManagementRadioGroupSelectionChanged(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void houmonKangoKaigoTimeSelectionChanged(ListSelectionEvent e) throws Exception;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void specialManagementRadioGroupSelectionChanged(ListSelectionEvent e) throws Exception;
