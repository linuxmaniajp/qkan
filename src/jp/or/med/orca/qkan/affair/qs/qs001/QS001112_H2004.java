...
/**
 * サービスパターン短期入所療養介護（診療所療養）(QS001112_H2004) 
 */
public class QS001112_H2004 extends QS001112_H2004Event {
  /**
   * コンストラクタです。
   */
  public QS001112_H2004(){
  }
  ...
  public void providerSelected(VRMap provider) throws Exception {
      ...
      if (provider != null) {
          ...
          // 療養環境基準（事業所パネル）KEY：1230203の値をチェックする。
          switch (ACCastUtilities.toInt(VRBindPathParser.get("1230203",
                  provider), 0)) {
          case 1:
              // 値が1（基準型）だった場合
              // defaultMapに KEY：1230205 VALUE：1（基準型）を設定する。
              VRBindPathParser.set("1230205", defaultMap, new Integer(1));
              break;
          case 0:
          case 2:
              // 値が0（未選択）もしくは、値が2（減算型Ⅰ）だった場合
              // defaultMapに KEY：1230205 VALUE：0（未設定）を設定する。
              // 過去の体制からの値なので値を振り替える
              VRBindPathParser.set("1230205", defaultMap, new Integer(0));
              break;
          case 3:
              // 値が3（減算型Ⅱ）だった場合
              // defaultMapに KEY：1230205 VALUE：3（減算型Ⅱ）を設定する。
              VRBindPathParser.set("1230205", defaultMap, new Integer(3));
              break;
          }
          // 設備環境減算
          switch (ACCastUtilities.toInt(VRBindPathParser.get("1230218",
                  provider), 0)) {
          case 0:
              // 値が0（未設定）だった場合
              // defaultMapに KEY：1230252 VALUE：0（未選択）を設定する。
              VRBindPathParser.set("1230252", defaultMap, new Integer(0));
              break;
          case 1:
              // 値が1（基準型）だった場合
              // defaultMapに KEY：1230252 VALUE：1（基準型）を設定する。
              VRBindPathParser.set("1230252", defaultMap, new Integer(1));
              break;
          case 2:
              // 値が2（減算型）だった場合
              // defaultMapに KEY：1230252 VALUE：2（減算型）を設定する。
              VRBindPathParser.set("1230252", defaultMap, new Integer(2));
              break;
          }
          // 人員減算
