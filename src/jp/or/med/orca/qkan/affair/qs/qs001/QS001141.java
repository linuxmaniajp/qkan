 ...
 * サブシステム 予定管理 (S)
 * プロセス サービス予定 (001)
 * プログラム サービスパターン介護予防短期入所療養介護（認知症） (QS001141)
 *
 *****************************************************************
 */
...
/**
 * サービスパターン介護予防短期入所療養介護（認知症）(QS001141) 
 */
public class QS001141 extends QS001141Event {
  /**
   * コンストラクタです。
   */
  public QS001141(){
  ...
  public static void main(String[] args) {
    ...
    ACFrame.debugStart(new ACAffairInfo(QS001141.class.getName(), param));
  }
  ...
  /**
    ...
    public void providerSelected(VRMap provider) throws Exception {
        ...
        if (provider != null) {
            ...
            // 栄養管理の評価（事業所パネル）KEY：1260306の値をチェックする。
            switch (ACCastUtilities.toInt(VRBindPathParser.get("1260306",
                    provider), 0)) {
            case 1:
                // 値が1（なし）だった場合
                // defaultMapに KEY：1260306 VALUE：1（なし）を設定する。
                VRBindPathParser.set("1260306", defaultMap, new Integer(1));
                break;
            case 2:
                // 値が2（栄養士）だった場合
                // defaultMapに KEY：1260306 VALUE：3（栄養士）を設定する。
                VRBindPathParser.set("1260306", defaultMap, new Integer(3));
                break;
            case 3:
                // 値が3（管理栄養士）だった場合
                // defaultMapに KEY：1260306 VALUE：2（管理栄養士）を設定する。
                VRBindPathParser.set("1260306", defaultMap, new Integer(2));
                break;
            }
            // 　　　※展開
            // 　　　　自身(this)にdefaultMapに設定する。
