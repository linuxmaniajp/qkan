...
/**
 * サービスパターン短期入所療養介護（介護医療院）イベント定義(QS001_204211_201804) 
 */
public abstract class QS001_204211_201804Event extends QS001_204211_201804State implements QS001Service {
  /**
   * コンストラクタです。
   */
  public QS001_204211_201804Event(){
    addEvents();
  }
  /**
   * イベント発生条件を定義します。
  ...
  protected void addEvents() {
    ...
    getShortStayHospitalRecuperationType().addListSelectionListener(new ListSelectionListener(){
        private boolean lockFlag = false;
        public void valueChanged(ListSelectionEvent e) {
            if (lockFlag) {
                return;
            ...
            lockFlag = true;
            try {
                shortStayHospitalRecuperationTypeSelectionChanged(e);
            }catch(Throwable ex){
                ACCommon.getInstance().showExceptionMessage(ex);
            }finally{
                lockFlag = false;
   ...
   * @throws Exception 処理例外
   */
  protected abstract void shortStayHospitalRecuperationTypeSelectionChanged(ListSelectionEvent e) throws Exception;

  //変数定義

  public static final int SYSTEM_SERVICE_KIND_DETAIL = 204211;
