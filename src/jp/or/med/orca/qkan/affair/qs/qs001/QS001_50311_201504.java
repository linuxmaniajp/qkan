...
/**
 * サービスパターン訪問型サービス(独自定率)(QS001_50311_201504)
 */
public class QS001_50311_201504 extends QS001_50311_201504Event {
	/**
	 * コンストラクタです。
	 */
	public QS001_50311_201504() {
	}
	...
	public static void main(String[] args) {
		...
		ACFrame.debugStart(new ACAffairInfo(QS001_50311_201504.class.getName(),
				param));
	}
	...
	public VRMap getValidData() throws Exception {
		...
		if ((!getSoA3BeginTime().isValidDate())
				|| (!getSoA3EndTime().isValidDate())) {
		...
		// ※返却用レコードから不要なキーを除去
		if (ACTextUtilities.isNullText(getSoA3BeginTime().getText())) {
			// 開始時刻コンボが空欄の場合
			// 返却用レコードから、開始時刻コンボのbindPathを除去する。
			data.remove(getSoA3BeginTime().getBindPath());
		}
		if (ACTextUtilities.isNullText(getSoA3EndTime().getText())) {
			// 終了時刻コンボが空欄の場合
			// 返却用レコードから、終了時刻コンボのbindPathを除去する。
			data.remove(getSoA3EndTime().getBindPath());
		}
		// サービス一覧のデータを追加
