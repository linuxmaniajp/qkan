...
/**
 * 提供票別表(QS00304) 
 * 帳票定義体ファイル名 ： QS00304.xml
 */
public class QS003P04 extends QS003P04Event {
  /**
   * コンストラクタです。
   */
  public QS003P04(){
  ...
  public boolean doPrint(ACChotarouXMLWriter writer, VRMap printParam) throws Exception {
    ...
    ACChotarouXMLUtilities.getInstance().addFormat(writer, "mainFormat", "QS00304.xml");
    //ページ開始
    ...
    // 　　印刷対象の事業所の事業所番号格納用の文字列 targetProviderID を生成する。
    String targetProviderID = null;
    // 　　serviceDataに、渡りパラメータの KEY : SERVICE_DATA の VALUE を設定する。
    // 　　targetProviderID に、渡りパラメータの KEY : TARGET_PROVIDER_ID を設定する。
    // 　　表内データの集計処理を行う。
    tabulation(serviceData, getPrintList(), getFooterMap(), targetProviderID);
   ...
   * @throws Exception 処理例外
   */
  public VRList tabulation(VRList serviceData, VRList printList, VRMap footerMap, String 事業所番号) throws Exception{
...
//    QkanSystemInformation.getInstance().getMasterService();
    ...
    // 　　対象事業所番号 targetProviderID と recordNow の KEY : PROVIDER_ID の VALUE を比較する。
    // 　　　等しい場合
    ...
    // 　　　等しくない場合
    // 　　　　他事業所レコード anotherProvider に recordNow を加算する。
    addRecordValueToAnotherProvider(anotherProvider, recordNow);

    // 　　過去レコード recordOld に現レコード recordNow を上書する。
    // 　小計清算処理を行う。
    addSubTotalToPrintList(getPrintList(), getFooterMap(), subTotal, recordNow);

    // 　印刷用レコード集合 printList に対象外レコード集合 excludeList を追加する。(パターンE)
    ...
    // 　印刷用レコード集合 printList に他事業所レコード anotherProvider を追加する。(パターンD)
    addRecordToPrintListPatternD(getPrintList(), anotherProvider);

    // 　フッターレコード footerMap に 他事業所レコード anotherProvider を加算する。
    addRecordValueToFooter(getFooterMap(), anotherProvider);
...
return null;
  ...
  /**
   * 「他事業所加算処理」に関する処理を行ないます。
   * @throws Exception 処理例外
   */
  public void addRecordValueToAnotherProvider(VRMap anotherProvider, VRMap recNow) throws Exception{
    // ■他事業所加算処理
    // 　recNow の KEY : LIMIT_AMOUNT_OBJECT の VALUE をチェックする。
    // 　　1(対象内)の場合
    // 　　　subTotal に recNow の下記 KEY の VALUE を加算する。(VALUE 同士を数値にキャストして足し算する。)
    // 　　　　SERVICE_UNIT_COST
    // 　　　　KUBUN_LIMIT_OVER
