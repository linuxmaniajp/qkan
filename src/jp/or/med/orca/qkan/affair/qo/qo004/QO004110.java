 ...
 */
public class QO004110 extends QO004110Event {
  /**
   * コンストラクタです。
   */
  public QO004110(){
  ...
  public static void main(String[] args) {
    ...
    ACFrame.debugStart(new ACAffairInfo(QO004110.class.getName(), param));
  }
...
//	}
  ...
  public void initialize() throws Exception{
    ...
    // ※初期値設定
    // 割引率テキストに初期値0を代入する。
	// 施設区分の初期値として「介護老人保健施設」を選択する。
	// 夜間勤務条件基準の初期値として「基準型」を選択する。
	// リハビリテーション機能強化の初期値として「なし」を選択する。
	...
	getFacilitiesDivision().setSelectedIndex(FACILITY_TYPE_NORMAL_ROUKEN);
    getNightWorkDivision().setSelectedIndex(1);
    getRehabilitationReinforcement().setSelectedIndex(1);
...
//    getRecuperatDinner().setSelectedIndex(1);
  ...
  public boolean isValidInput() throws Exception{
    ...
    // ・rehabilitationReinforcement（リハビリテーション機能強化ラジオグループ）
    // ※errMsg = リハビリテーション機能強化
    if(!getRehabilitationReinforcement().isSelected()){
    	errMsg = "リハビリテーション機能強化";
    	QkanMessageList.getInstance().ERROR_OF_NEED_CHECK_FOR_SELECT(errMsg);
    	getRehabilitationReinforcement().requestFocus();
    	return false;
    }
...
//    }
  ...
  public void setStateByFacilitiesDivision() throws Exception{
	...
	// 施設区分の値をチェックする。
	if (getFacilitiesDivision().getSelectedIndex() == FACILITY_TYPE_NORMAL_ROUKEN) {
		// 「介護老人保健施設」が選択された場合
