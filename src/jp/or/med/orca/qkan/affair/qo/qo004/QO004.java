...
public class QO004 extends QO004Event {
  ...
  protected void initAction(ACAffairInfo affair) throws Exception {
	...
	if(VRBindPathParser.has("PROCESS_MODE", param)){
		setPROCESS_MODE(ACCastUtilities.toInt(VRBindPathParser.get("PROCESS_MODE", param)));
	}
	
	if(VRBindPathParser.has("PROVIDER_ID", param)){
		setSelectedProviderId(ACCastUtilities.toString(VRBindPathParser.get("PROVIDER_ID", param)));
	}
  ...
  public boolean canBack(VRMap parameters) throws Exception {
	...
	if(getPROCESS_MODE() == QkanConstants.PROCESS_MODE_UPDATE){
		...
		VRBindPathParser.set("PROVIDER_ID", parameters, getSelectedProviderId());
	}
  ...
  /**
	 ...
	 * @throws Exception 処理例外
	 */
  protected void providerStaffTableSelectionChanged(ListSelectionEvent e) throws Exception{
   ...
   * @throws Exception 処理例外
   */
  protected void serviceKindTableSelectionChanged(ListSelectionEvent e) throws Exception{
	...
	// 選択行レコード取得
	VRMap selectedService = (VRMap)getServiceKindTable().getSelectedModelRowValue();
	// 選択レコードのサービス種類を取得
	int selectedServiceType = ACCastUtilities.toInt(
			VRBindPathParser.get("SYSTEM_SERVICE_KIND_DETAIL", selectedService));
	...
	if(isModifiedOnService()){
		...
		switch(QkanMessageList.getInstance().QO004_WARNING_OF_SERVICE_INSERT()){
		// 「YES」が選択された場合
		case ACMessageBox.RESULT_YES:
			// 反映処理
			if(!reflect()){
				// 選択を元に戻す
				VRList temp = (VRList)getProviderServiceTableModel().getAdaptee();
				int i = ACBindUtilities.getMatchIndexFromValue(temp, 
						"SYSTEM_SERVICE_KIND_DETAIL", new Integer(getCurrentServiceType()));
    ...
    if(panel == null){
	    ...
	    getProviderDetailServiceDetails().add((ACPanel)getProviderServiceClass(), VRLayout.CLIENT);
   ...
   * @throws Exception 処理例外
   */
  protected void providerStaffInsertActionPerformed(ActionEvent e) throws Exception{
    // 担当者を追加する処理
    ...
    // 追加行を選択状態に設定する。
    getProviderStaffTable().setSelectedModelRow(getProviderStaffList().size() - 1);
   ...
   * @throws Exception 処理例外
   */
  protected void providerStaffFindActionPerformed(ActionEvent e) throws Exception{
	...
	// 選択されているレコードを取得
	VRMap selectedMap = (VRMap)getProviderStaffTable().getSelectedModelRowValue();
	
	// 選択レコードより、画面上にないデータを移す。
	int staffId = ACCastUtilities.toInt(VRBindPathParser.get("STAFF_ID", selectedMap));
	VRBindPathParser.set("STAFF_ID", map, new Integer(staffId));
	
	// 選択レコードと画面上のデータを置き換える。
	getProviderStaffList().setData(getProviderStaffTable().getSelectedModelRow(), map);
   ...
   * @throws Exception 処理例外
   */
  protected void providerStaffDeleteActionPerformed(ActionEvent e) throws Exception{
  	...
  	if(!(getProviderStaffList() == null || getProviderStaffList().size() == 0)){
	    ...
	    switch(QkanMessageList.getInstance().WARNING_OF_DELETE_SELECTION()){
	    	...
	    	case ACMessageBox.RESULT_OK:
	    		...
	    		getProviderStaffList().remove(getProviderStaffTable().getSelectedModelRow());			
	    		
			    // 削除された行の1行上の行を選択する。
			    getProviderStaffTable().setSelectedSortedRowOnAfterDelete(sortedRow);
...
//    ACFrame.getInstance().setFrameEventProcesser(new QkanFrameEventProcesser());
  ...
  public void initialize() throws Exception{
	...
	getServiceSnapshot().setRootContainer(getProviderDetailServiceDetails());
	...
	// 郵便番号から住所変換
    // 郵便番号と住所を関連付ける。
	new ACZipRelation(getProviderPostnoOn(),getProviderPostnoUnder(),getProviderAddress());
  ...
  public void affairSetUp() throws Exception{
	...
	temp = QkanCommon.getArrayFromMasterCode(CODE_PROVIDER_TYPE, "PROVIDER_TYPE");
	VRBindPathParser.set("PROVIDER_TYPE", comboMap, temp);
	
	// ・CODE_ID = CODE_LAW_VERSION　※法改正区分
	temp = QkanCommon.getArrayFromMasterCode(CODE_LAW_VERSION, "LAW_VERSION");
  ...
  public void doFind() throws Exception{
	...
	if(getPROCESS_MODE() == QkanConstants.PROCESS_MODE_UPDATE){
	    ...
	    setProviderList(QkanCommon.getProviderInfo(getDBManager(), getSelectedProviderId()));	    
	    
	    // パッシブチェック用のデータを登録する。
	    getPassiveChecker().reservedPassive(getPROVIDER_PASSIVE_CHECK_KEY(), getProviderList());
  ...
  public void doCreateServiceTableData() throws Exception{
	...
	// サービスマスタデータを取得する。
	VRMap temp = QkanCommon.getMasterService(getDBManager(), TARGET_DATE_20060401);
    ...
    for(int i = 0; i < getProviderServiceList().size(); i++){
    	...
    	VRMap detail = ACBindUtilities.getMatchRowFromMap(
    			providerServiceDetailList, "SYSTEM_SERVICE_KIND_DETAIL", service);
  ...
  public void doCreateStaffData() throws Exception{
	...
	if(getPROCESS_MODE() == QkanConstants.PROCESS_MODE_INSERT){
	    ...
	    if(getProviderStaffList() == null || getProviderStaffList().size() == 0){
	    	...
	    	setSTAFF_PROCESS_MODE(QkanConstants.PROCESS_MODE_INSERT);
	    	// 担当者情報領域の初期値作成
	    	VRMap defaultMap = (VRMap)getProviderStaffInputPanel().createSource();
	    	// 領域のクリア
  ...
  public boolean isValidInputStaff(int checkMode) throws Exception{
	...
	if(!ACTextUtilities.isNullText(getProviderStaffCareManagerNo().getText())){
		...
		for(int i = 0; i < getProviderStaffList().size(); i++){
			...
			if(careManagerNo.equals(ACCastUtilities.toString(VRBindPathParser.get("CARE_MANAGER_NO", temp)))){
				getProviderStaffCareManagerNo().requestFocus();
	...
	if(ACTextUtilities.isNullText(getProviderStaffFamilyName().getText()) 
			&& ACTextUtilities.isNullText(getProviderStaffFirstName().getText())){
	...
	try{
		...
		if(getPROCESS_MODE() == QkanConstants.PROCESS_MODE_INSERT){
			...
			if(!(list == null || list.size() == 0)){
				...
				errMsg = "事業所番号";
				errMsg2 = "事業所";
				switch(QkanMessageList.getInstance().WARNING_OF_RESTORATION_OF_DELETE_DATA(errMsg, errMsg2)){
				case ACMessageBox.RESULT_OK:
					// 更新モードに切り替える
					setPROCESS_MODE(QkanConstants.PROCESS_MODE_UPDATE);
					// 事業所番号を設定
					setSelectedProviderId(ACCastUtilities.toString(
							VRBindPathParser.get("PROVIDER_ID", affairData)));
					// フラグの初期化
					clearFlags();
					// 画面セットアップ
					affairSetUp();
	    ...
	    if(getPROCESS_MODE() == QkanConstants.PROCESS_MODE_INSERT){
		    // PROCESS_MODEが、共通定数の｢PROCESS_MODE_INSERT(4)｣だった場合
		    // 登録用パッシブタスクを登録する。
	    	getPassiveChecker().addPassiveInsertTask(getPROVIDER_PASSIVE_CHECK_KEY(), affairData);
	    }else{
		    // PROCESS_MODEが、共通定数の｢PROCESS_MODE_UPDATE(3)｣だった場合
		    // 更新用パッシブタスクを登録する。
	    	getPassiveChecker().addPassiveUpdateTask(getPROVIDER_PASSIVE_CHECK_KEY(), 0);
	    }
	    ...
	    if(!getPassiveChecker().passiveCheck(getDBManager())){	    	
	    	...
	    	if(getPROCESS_MODE() == QkanConstants.PROCESS_MODE_INSERT){
	    		...
	    		errMsg = "事業所番号";
			    QkanMessageList.getInstance().ERROR_OF_SAME_DATA_FOUND(errMsg);
			    getProviderId().requestFocus();
	    	}else{
			    // PROCESS_MODEが、共通定数の｢PROCESS_MODE_UPDATE(3)｣だった場合
	    ...
	    if(getStaffDataExistenceFlag() == 1){
	    	...
	    	VRBindPathParser.set(
	    			"PROVIDER_ID", staffMap, getSelectedProviderId());
	    ...
	    if(!(getProviderStaffList() == null || getProviderStaffList().size() == 0)){
		    for(int i = 0; i < getProviderStaffList().size(); i++){
		    	...
		    	// staffMapに下記のKEY/VALUEを設定する。	
		    	// KEY：PROVIDER_ID　VALUE：selectedProviderId
		    	VRBindPathParser.set("PROVIDER_ID", staffMap, getSelectedProviderId());
	    ...
	    for(int i = 0; i < getProviderServiceList().size(); i++){
	    	...
	    	boolean offer = ACCastUtilities.toBoolean(
	    			VRBindPathParser.get("OFFER", temp));
	    	...
	    	if(offer){
	    		...
	    		if(systemServiceKindDetail == SERVICE_TYPE_KAIGO_KANGO){
	    			...
	    			VRMap iryoKango = (VRMap)VRBindPathParser.get(
	    					...
	    					(VRMap)QkanCommon.getMasterService(getDBManager()));
	    ...
	    if(insertServiceList != null){
	    	...
	    	int jijigyoshoType = ACCastUtilities.toInt(
		    		VRBindPathParser.get("PROVIDER_JIJIGYOUSHO_TYPE", affairData));
		    ...
		    final String[] removeKeys = new String[]{
			    	...
			    	"SERVICE_ABBREVIATION",
			    	"SERVICE_KIND_NAME",
			    	"SERVICE_CALENDAR_ABBREVIATION",
			    	"CLAIM_STYLE_TYPE",
			    	"BUSINESS_TYPE",
			    	"CALENDAR_PASTE_FLAG",
			    	"CHANGES_CONTENT_TYPE",
			    	"CLAIM_LAYER",
			    	"SERVICE_FAMILY",
			    	"SERVICE_SORT",
			    	"LAST_TIME",
		    ...
		    for(int i = 0; i < insertServiceList.size(); i++){
		    	VRBindPathParser.set("PROVIDER_ID", 
		    			(VRMap)insertServiceList.get(i), getSelectedProviderId());
		    }
		    
		    // 事業所サービス詳細情報の登録を行う。
		    QkanCommon.updateProviderServiceDetail(
		    		getDBManager(), insertServiceList, getSelectedProviderId());
  ...
  public boolean doCheckDeleteService() throws Exception{
    ...
    // sqlParamに下記の値を格納する。
    // KEY：SYSTEM_SERVICE_KIND_DETAIL　VALUE：現在表示しているパネルのSYSTEM_SERVICE_KIND_DETAIL
    // KEY：PROVIDER_ID　　VALUE：selectedProviderId
    sqlParam = new VRHashMap();    
    VRBindPathParser.set("SYSTEM_SERVICE_KIND_DETAIL", 
    		sqlParam, new Integer(systemServiceKindDetail));
  ...
  public void insertProviderMenu(VRList list) throws Exception{
	...
	try{
		...
		for(int i = 0; i < list.size(); i++){
			...
			businessTypeTemp = ACCastUtilities.toInt(
					VRBindPathParser.get("BUSINESS_TYPE", temp));
		...
		for(int i = 0; i < getMasterMenuList().size(); i++){
		    ...
		    businessTypeMaster = ACCastUtilities.toInt(
		    		VRBindPathParser.get("BUSINESS_TYPE", masterMenuMap));
		    ...
		    if((businessTypeMaster & businessTypeSum) == 0){
		    	...
		    	VRBindPathParser.set(
		    			"PROVIDER_ID", masterMenuMap, getSelectedProviderId());
		    	VRBindPathParser.set(
		    			"SHOW_FLAG", masterMenuMap, new Integer(0));
		    }else{
			    // 計算した値が0以外の場合
			    // masterMenuMapに下記のKEY/VALUEを設定する。
			    // KEY：PROVIDER_ID　　VALUE：selectedProviderId
		    	...
		    	VRBindPathParser.set(
		    			"PROVIDER_ID", masterMenuMap, getSelectedProviderId());
		    	VRBindPathParser.set(
		    			"SHOW_FLAG", masterMenuMap, new Integer(1));
		    }
		    ...
		    if(getSelectedProviderId().equals(QkanSystemInformation.getInstance().getLoginProviderID())){
  ...
  public boolean doValidProviderCheck() throws Exception{
    ...
    if(ACTextUtilities.isNullText(getProviderId().getText())){
    	getProviderId().requestFocus();
    	errMsg1 = "事業所番号";
    	QkanMessageList.getInstance().ERROR_OF_NEED_CHECK_FOR_INPUT(errMsg1);
    	return false;
    }
    ...
    if(!getProviderMainContentDivision().isSelected()){
    	...
    	errMsg1 = "自事業所区分";
    	QkanMessageList.getInstance().ERROR_OF_NEED_CHECK_FOR_SELECT(errMsg1);
    	return false;    	
    }
...
//	    	return new QO004020();
  ...
  public boolean isModifiedOnService( ) throws Exception{
	  ...
	  // 提供チェックの変更チェック
	  boolean oldOffer = ACCastUtilities.toBoolean(
			  VRBindPathParser.get("OFFER", getServiceInfos(getCurrentServiceType())));
