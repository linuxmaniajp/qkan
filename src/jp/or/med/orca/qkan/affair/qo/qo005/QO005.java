...
public class QO005 extends QO005Event {
	 ...
	 *             処理例外
	 */
	protected void serverSelectRadiosActionPerformed(ActionEvent e) throws Exception {
		// ※画面の状態設定
	 ...
	 *             処理例外
	 */
	protected void dbFileSelectFileCompareButtonActionPerformed(ActionEvent e) throws Exception {
		// ※ファイル選択処理
		...
		selectFile = DBfileChooser.showOpenDialog(defaultPath , "","使用するデータベースファイルを指定して下さい。", DBFilefilter.getFileExtensions(), DBFilefilter.getDescription());
		...
		// 取得したファイルから拡張子を補完する
		selectFile = new File(selectFile.getParent(), getFBFilePathWithExtension(selectFile.getName(), 1, FB_FILE_EXTENSIONS));
		...
		// 許可文字コード
		if (!VRCharType.ONLY_HALF_CHAR.isMatch(String.valueOf(String.valueOf(selectFile)))) {
			getDbFileSelectFileName().requestFocus();
			QkanMessageList.getInstance().QO005_ERROR_OF_DBPATH_INCLUDE_JAPANESE_CHAR();
			return;
		}
	...
	protected void backupActionPerformed(ActionEvent e) throws Exception {
		...
		if (!displayPath.equals(properityPath)) {
			QkanMessageList.getInstance().QO005_ERROR_OF_NOT_SAME_PROPERTY_DBPATH();
			return;
		}
		...
		String backupPath = new File(getProperty("DBConfig/Path")).getParent() + FILE_SEPARATOR + "QKANOLD.FDB";
		File saveFile = null;

		// ※ファイル選択処理
		// ファイル選択ダイアログ(保存)を表示する。
		saveFile = DBfileChooser.showSaveDialog(new File(getProperty("DBConfig/Path")).getParent(), getDefaultFileName("old"), "データベースファイルの保存場所と退避ファイル名を指定して下さい。", DBFilefilter.getFileExtensions(), DBFilefilter.getDescription());

		if (!(saveFile == null)) {

			// ファイルが選択された場合
			// 取得したファイルから拡張子を補完する
			saveFile = new File(saveFile.getParent(), getFBFilePathWithExtension(saveFile.getName(), 1, FB_FILE_EXTENSIONS));
		...
		if (saveFile.exists()) {
			...
			if (QkanMessageList.getInstance().QO005_QUESTION_OF_EXIST_SELECTED_FDB() == SELECT_CANCEL) {
				// 「キャンセル」選択時
				// 「※ファイル選択ダイアログ」に戻る。(№9)
				return;
			}
	...
	protected void restoreActionPerformed(ActionEvent e) throws Exception {
		...
		restoreFile = DBfileChooser.showOpenDialog(new File(getProperty("DBConfig/Path")).getParent(), "", "退避したデータベースファイルを指定して下さい。", DBFilefilter.getFileExtensions(), DBFilefilter.getDescription());

		if (!(restoreFile == null)) {

			// ファイルが選択された場合
			// 取得したファイルから拡張子を補完する
			restoreFile = new File(restoreFile.getParent(), getFBFilePathWithExtension(restoreFile.getName(), 1, FB_FILE_EXTENSIONS));
		...
		if (restorePath.equals(getProperty("DBConfig/Path"))) {
			...
			QkanMessageList.getInstance().QO005_ERROR_OF_BACKUP_SAME_TO_RESTORE();
			// 「※ファイル選択ダイアログ」に戻る。(№12)
			return;
		}

		// DB接続をチェックする為にレストアファイルをテンポラリ領域にコピーしてチェックを行う
		checkPath = new File(getProperty("DBConfig/Path")).getParent() + FILE_SEPARATOR + "QKANCHECK.FDB";
		try {
			...
			QkanMessageList.getInstance().QO005_ERROR_OF_COPY_DBFILE_TEMP_FAILED();
			return;
		}
		...
		checkParams.setData("SERVER_SELECT_RADIOS", new Integer(SERVER_RADIO_LOCAL));
		if (!checkDBConnect(checkParams)) {
			// 接続チェックで接続できなかった場合
		...
		try {
			...
			QkanMessageList.getInstance().QO005_ERROR_OF_DELETE_DBFILE_TEMP_FAILED();
			return;
		}
		...
		// 
		backupPath = new File(getProperty("DBConfig/Path")).getParent() + FILE_SEPARATOR + "QKANOLD.FDB";
		File backupFile = new File(backupPath);

		// でも、リストア元が実はQKANOLD.FDBだったなんて場合
		if (backupFile.equals(restoreFile)) {
			// バックアップする為のファイルはOLD2とか変えてみる
			backupPath = new File(getProperty("DBConfig/Path")).getParent() + FILE_SEPARATOR + "QKANOLD2.FDB";
		...
		// 「OK」を選択した場合
		// 処理を継続する。
		if (QkanMessageList.getInstance().QO005_QUESTION_OF_BACKUP_FILENAME(restorePath, backupFile.getAbsolutePath()) == SELECT_CANCEL) {
			// 「キャンセル」を選択した場合
			// 処理を中断する。
			return;
		}
		...
		if (backupFile.exists()) {
			...
			if (QkanMessageList.getInstance().QO005_QUESTION_OF_EXIST_SELECTED_FDB() == SELECT_CANCEL) {
				...
				// ファイル選択ダイアログ(保存)を表示する。
				backupFile = backupFileChooser.showSaveDialog(new File(getProperty("DBConfig/Path")).getParent(), "", "データベースファイルの保存場所と退避ファイル名を指定して下さい。", DBFilefilter.getFileExtensions(), DBFilefilter.getDescription());

				if (!(backupFile == null)) {

					// ファイルが選択された場合
					// 取得したファイルから拡張子を補完する
					backupFile = new File(backupFile.getParent(), getFBFilePathWithExtension(backupFile.getName(), 1, FB_FILE_EXTENSIONS));
	 ...
	 *             処理例外
	 */
	protected void pdfFileSelectFileCompareButtonActionPerformed(ActionEvent e) throws Exception {
		// ※ファイル選択処理
		...
		// ファイル選択ダイアログを開く。
		pdfViewerExecuteFile = pdffileChooser.showOpenDialog("使用するPDFファイル閲覧プログラムを指定してください。", pdfFilefilter.getFileExtensions(), pdfFilefilter.getDescription());
		...
		// 取得したファイルから拡張子を補完する
		pdfViewerExecuteFile = new File(pdfViewerExecuteFile.getParent(), getPDFFilePathWithExtension(pdfViewerExecuteFile.getName(), 0, PDF_FILE_EXTENSIONS));
...
//      税率を再設定する。
	...
	public VRMap loadSettingFile() throws Exception {
		...
		if (ACFrame.getInstance().hasProperty("DBConfig/Path")) {
			params.setData("DB_FILE_SELECT_FILE_NAME", getProperty("DBConfig/Path"));
		} else {
			params.setData("DB_FILE_SELECT_FILE_NAME", new String("DATABASE_PATH"));
		}
		// 設定ファイルの ID : DBConfig/Server の値をチェックする。
		if (ACFrame.getInstance().hasProperty("DBConfig/Server")) {
			if ("localhost".equals(getProperty("DBConfig/Server")) | "127.0.0.1".equals(getProperty("DBConfig/Server"))) {
				// 「localhost」の場合
				...
				// paramsの KEY : SERVER_SELECT_RADIOS のVALUEに 1 を設定する。
				// paramsの KEY : SERVER_SELECT_IP_TEXT のVALUEに ""(ブランク) を設定する。
				params.setData("SERVER_SELECT_RADIOS", new Integer(SERVER_RADIO_LOCAL));
				params.setData("SERVER_SELECT_IP_TEXT", new String(""));
			} else {
				// 上記以外の場合
				// paramsの KEY : SERVER_SELECT_RADIOS のVALUEに 2 を設定する。
				params.setData("SERVER_SELECT_RADIOS", new Integer(SERVER_RADIO_REMOTE));
				// paramsの KEY : SERVER_SELECT_IP_TEXT のVALUEに、設定ファイルの ID :
				// DBConfig/Server の値を設定する。
				params.setData("SERVER_SELECT_IP_TEXT", getProperty("DBConfig/Server"));
			}
		} else {
			// デフォルト値の設定を行う
			params.setData("SERVER_SELECT_RADIOS", new Integer(SERVER_RADIO_LOCAL));
		...
		if (ACFrame.getInstance().hasProperty("Acrobat/Path")) {
			...
			params.setData("PDF_FILE_SELECT_FILE_NAME", getProperty("Acrobat/Path"));
		} else {
		...
		if (ACFrame.getInstance().hasProperty("PrintConfig/YoshikiBango")) {
			...
			params.setData("YOSHIKI_BANGO", getProperty("PrintConfig/YoshikiBango"));
		} else {
			params.setData("YOSHIKI_BANGO", new Integer(CHECKBOX_ON));
		}

		if (ACFrame.getInstance().hasProperty("PrintConfig/NinchishoTokuteiShisetsu")) {
			// paramsの KEY : NINCHISHO_TOKUTEI_SHISETSU のVALUEに、設定ファイルの ID :
			// PrintConfig/ninchishoTokuteiShisetsu の値を設定する。
			params.setData("NINCHISHO_TOKUTEI_SHISETSU", getProperty("PrintConfig/NinchishoTokuteiShisetsu"));
		} else {
			params.setData("NINCHISHO_TOKUTEI_SHISETSU", new Integer(CHECKBOX_ON));
		}
		...
		if (ACFrame.getInstance().hasProperty("PrintConfig/ShisetsuService")) {
			...
			params.setData("SHISETSU_SERVICE", getProperty("PrintConfig/ShisetsuService"));
		} else {
		...
		if (ACFrame.getInstance().hasProperty("PrintConfig/HomeMedicalAdvice")) {
			...
			params.setData("PRINT_HOME_MEDICAL_ADVICE", getProperty("PrintConfig/HomeMedicalAdvice"));
		} else {
			params.setData("PRINT_HOME_MEDICAL_ADVICE", new Integer(CHECKBOX_OFF));
		}
		...
		if (ACFrame.getInstance().hasProperty("PrintConfig/OncePerMonth")) {
			...
			params.setData("ONCE_PER_MONTH", getProperty("PrintConfig/OncePerMonth"));
		} else {
		...
		if (ACFrame.getInstance().hasProperty("PrintConfig/PrivateExpenses")) {
			...
			params.setData("PRIVATE_EXPENSES", getProperty("PrintConfig/PrivateExpenses"));
		} else {
	...
	public boolean saveSettingFile(VRMap params) throws Exception {
		...
		if (checkValue(params)) {
			...
			setProperty("DBConfig/Path", String.valueOf(params.getData("DB_FILE_SELECT_FILE_NAME")));

			// paramsの KEY : DB_FILE_SELECT_FILE_NAME の VALUE をチェックする。
			if (params.getData("SERVER_SELECT_RADIOS").equals(new Integer(SERVER_RADIO_LOCAL))) {
				// 1の場合
				// 設定ファイルの ID : DBConfig/Server の値に、「localhost」を設定する。
				setProperty("DBConfig/Server", "localhost");
			} else if (params.getData("SERVER_SELECT_RADIOS").equals(new Integer(SERVER_RADIO_REMOTE))) {
				// 2の場合
				// 設定ファイルの ID : DBConfig/Server の値に、paramsの KEY :
				// SERVER_SELECT_IP_TEXT の VALUE を設定する。
				setProperty("DBConfig/Server", String.valueOf(params.getData("SERVER_SELECT_IP_TEXT")));
			}
			...
			// のVALUEを設定する。
			setProperty("Acrobat/Path", (String) params.getData("PDF_FILE_SELECT_FILE_NAME"));
			// 設定ファイルの ID : PrintConfig/yoshikiBango の値に、paramsの KEY :
			// YOSHIKI_BANGO
			// のVALUEを設定する。
			setProperty("PrintConfig/YoshikiBango", String.valueOf(params.getData("YOSHIKI_BANGO")));
			// 設定ファイルの ID : PrintConfig/ninchishoTokuteiShisetsu の値に、paramsの KEY
			// :
			// NINCHISHO_TOKUTEI_SHISETSU のVALUEを設定する。
			setProperty("PrintConfig/NinchishoTokuteiShisetsu", String.valueOf(params.getData("NINCHISHO_TOKUTEI_SHISETSU")));
			// 設定ファイルの ID : PrintConfig/shisetsuService の値に、paramsの KEY :
			// SHISETSU_SERVICE のVALUEを設定する。
			setProperty("PrintConfig/ShisetsuService", String.valueOf(params.getData("SHISETSU_SERVICE")));
			// 設定ファイルの ID : PrintConfig/HomeMedicalAdvice の値に、paramsの KEY :
			// PRINT_HOME_MEDICAL_ADVICE のVALUEを設定する。
			setProperty("PrintConfig/HomeMedicalAdvice", String.valueOf(params.getData("PRINT_HOME_MEDICAL_ADVICE")));
			// 設定ファイルの ID : PrintConfig/oncePerMonth の値に、paramsの KEY :
			// ONCE_PER_MONTH のVALUEを設定する。
			setProperty("PrintConfig/OncePerMonth", String.valueOf(params.getData("ONCE_PER_MONTH")));
	 ...
	 * @return 拡張子付きファイル名
	 */
	public String getFBFilePathWithExtension(String fileName, int defaultExtensionIndex, String[] extensions) {
		if (fileName == null) {
			return "";
		}
	 ...
	 * @return 拡張子付きファイル名
	 */
	public String getPDFFilePathWithExtension(String fileName, int defaultExtensionIndex, String[] extensions) {
		if (fileName == null) {
			return "";
		}
	...
	public boolean checkValue(VRMap params) throws Exception {
		// DBのIPアドレス未入力チェック
		if (params.getData("SERVER_SELECT_RADIOS").equals(new Integer(SERVER_RADIO_REMOTE))) {
			// サーバのラジオがリモートだった場合で、IPアドレスが入力されているかチェック
			if (ACTextUtilities.isNullText(params.getData("SERVER_SELECT_IP_TEXT"))) {
		...
		// ファイル名未入力
		if (ACTextUtilities.isNullText(params.getData("DB_FILE_SELECT_FILE_NAME"))) {
			QkanMessageList.getInstance().QO005_ERROR_OF_EMPTY_DBFILE_PATH();
			getDbFileSelectFileName().requestFocus();
			return false;
		}
		...
		// PDFファイル名未入力
		if (getPdfFileSelects().isVisible() && ACTextUtilities.isNullText(params.getData("PDF_FILE_SELECT_FILE_NAME"))) {
			QkanMessageList.getInstance().QO005_ERROR_OF_EMPTY_PDF_VIEWER_PATH();
			getPdfFileSelectFileName().requestFocus();
			return false;
		}

		// 許可文字コード
		if (!VRCharType.ONLY_HALF_CHAR.isMatch(String.valueOf(params.getData("DB_FILE_SELECT_FILE_NAME")))) {
			final String osName = String.valueOf(System.getProperty("os.name")).toLowerCase();
			if ((osName.indexOf("mac") < 0) || (getServerSelectRadioItem1().isSelected())) {
	 ...
	 *             処理例外
	 */
	public boolean checkDBConnect(VRMap params) throws Exception {
		// サーバのアドレスの格納
		BridgeFirebirdDBManager dbm = new BridgeFirebirdDBManager();
		try {
			String server;
			if (params.getData("SERVER_SELECT_RADIOS").equals(new Integer(SERVER_RADIO_LOCAL))) {
				...
				server = String.valueOf(params.getData("SERVER_SELECT_IP_TEXT"));
			}
			...
			String path = String.valueOf(params.getData("DB_FILE_SELECT_FILE_NAME"));
			int loginTimeout = ACCastUtilities.toInt(getProperty("DBConfig/LoginTimeOut"));
			int maxPoolSize = ACCastUtilities.toInt(getProperty("DBConfig/MaxPoolSize"));
			String charSet = getProperty("DBConfig/CharSet");

			dbm.releaseAll();
			dbm = new BridgeFirebirdDBManager(server, port, userName, passWord, path, loginTimeout, maxPoolSize, charSet);

			if (dbm.isAvailableDBConnection()) {

				VRList list = dbm.executeQuery("SELECT M_QKAN_VERSION.MASTER_DATA_VERSION FROM M_QKAN_VERSION");
