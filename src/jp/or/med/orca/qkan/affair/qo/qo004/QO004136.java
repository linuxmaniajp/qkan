 ...
 * 介護予防通所リハ(QO004136)
 */
public class QO004136 extends QO004136Event {
	/**
	 * コンストラクタです。
	 */
	public QO004136() {
	...
	public static void main(String[] args) {
		...
		ACFrame.debugStart(new ACAffairInfo(QO004136.class.getName(), param));
	}
	...
	public boolean isValidInput() throws Exception {
		...
		if (!getStaffLack().isSelected()) {
			// ・staffLack（職員の欠員による減算の状況ラジオグループ）※ errMsg = 職員の欠員による減算の状況
			errMsg = "人員減算";
			// 未選択だった場合
		...
		// 下記のラジオグループに対して選択チェックを行う。未選択だった場合は errMsg にメッセージを格納する。
		if (!getFunctionImprovement().isSelected()) {
			// ・functionImprovement（運動器機能向上体制ラジオグループ）※ errMsg = 運動器機能向上体制
			errMsg = "運動器機能向上体制";
			// 未選択だった場合
			// エラーメッセージを表示する。※メッセージID = ERROR_OF_NEED_CHECK_FOR_SELECT 引数 = errMsg
			QkanMessageList.getInstance().ERROR_OF_NEED_CHECK_FOR_SELECT(errMsg);
			// エラーが発生したインスタンスにフォーカスを当てる。
			getFunctionImprovement().requestFocus();
			// 処理を抜ける。（中断する）
			return false;
		...
		if (!getNourishmentImprovement().isSelected()) {
			// ・nourishmentImprovement（栄養改善体制ラジオグループ）※ errMsg = 栄養改善体制
			errMsg = "栄養改善体制";
			// 未選択だった場合
			// エラーメッセージを表示する。※メッセージID = ERROR_OF_NEED_CHECK_FOR_SELECT 引数 = errMsg
			QkanMessageList.getInstance().ERROR_OF_NEED_CHECK_FOR_SELECT(errMsg);
		...
		if (!getMouthImprovementAdd().isSelected()) {
			// ・mouthImprovementAdd（口腔機能向上体制ラジオグループ）※ errMsg = 口腔機能向上体制
			errMsg = "口腔機能向上体制";
			// 未選択だった場合
			// エラーメッセージを表示する。※メッセージID = ERROR_OF_NEED_CHECK_FOR_SELECT 引数 = errMsg
			QkanMessageList.getInstance().ERROR_OF_NEED_CHECK_FOR_SELECT(errMsg);
		...
		if (!getProviderEvaluation().isSelected()) {
			// ・providerEvaluation(事業所評価加算)※ errMsg = 事業所評価加算
			errMsg = "事業所評価体制";
			// 未選択だった場合
			// エラーメッセージを表示する。※メッセージID = ERROR_OF_NEED_CHECK_FOR_SELECT 引数 = errMsg
			QkanMessageList.getInstance().ERROR_OF_NEED_CHECK_FOR_SELECT(errMsg);
			// エラーが発生したインスタンスにフォーカスを当てる。
			getProviderEvaluation().requestFocus();
			// 処理を抜ける。（中断する）
			return false;
