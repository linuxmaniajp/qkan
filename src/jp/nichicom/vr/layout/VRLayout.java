...
public class VRLayout implements LayoutManager2 {
    ...
    /** 上下左右に配置 */
    private LinkedHashMap<Component, String> sideDocks = new LinkedHashMap<Component, String>();

    /** センター配置 */
    private ArrayList<Component> centerDocks = new ArrayList<Component>();

    /** 自動配置 */
    private LinkedHashMap<Component, String> flows = new LinkedHashMap<Component, String>();
     ...
     * 
     * @param align
     *            配置方向（LEFT, CENTER, RIGHT, LEADING, TRAILINGのいずれか）
     */
     ...
     *            垂直方向のグリッドの高さ
     * @param fitHgrid
     *            水平方向に最大まで幅を広げる場合はtrue
     * @param fitVgrid
     *            垂直方向に最大まで高さを広げる場合はtrue
     */
     ...
     *            ストレッチする場合はtrue
     * @param vAlign
     *            配置方向（TOP, CENTER, BOTTOM, LEADING, TRAILINGのいずれか）
     */
    public VRLayout(int align, int hgap, int vgap, int hgrid, int vgrid,
            boolean fitHgrid, boolean fitVgrid, int vLineAlign,
            boolean autoWrap, boolean labelOverwrap, int labelMargin,
     ...
     * @param vAlign
     *            配置方向（TOP, CENTER, BOTTOM, LEADING, TRAILINGのいずれか）
     * @param horizontalDivide
     *            クライアント領域を水平方向に分割する場合はtrue
     * @param fitHLast
     *            水平方向に最後まで幅を広げる場合はtrue
     * @param fitVLast
     *            垂直方向に最後まで高さを広げる場合はtrue
     */
    public VRLayout(int align, int hgap, int vgap, int hgrid, int vgrid,
            boolean fitHgrid, boolean fitVgrid, int vLineAlign,
            boolean autoWrap, boolean labelOverwrap, int labelMargin,
     ...
     * @return 削除したか
     */
    protected boolean removeComponent(Map<Component, String> map, Component comp) {
    ...
    public Dimension preferredLayoutSize(Container target) {
        ...
        synchronized (target.getTreeLock()) {
            ...
            Iterator<Component> it = sideDocks.keySet().iterator();
    ...
    public void layoutContainer(Container target) {
        synchronized (target.getTreeLock()) {
                ...
                if (visibleCount > 0) {
                    if (isHorizontalDivide()) {
                        ...
                        for (int i = 0; i <= centerDocks.size() - 1; i++) {
                            Component c = centerDocks.get(i);
                            if (c.isVisible()) {
                                c.setBounds(cx, cy + cHeight * visibleIndex++,
                                        cWidth, cHeight);
                            }
                        }
                    } else {
                        ...
                        for (int i = 0; i <= centerDocks.size() - 1; i++) {
                            Component c = centerDocks.get(i);
                            if (c.isVisible()) {
                                c.setBounds(cx + cWidth * visibleIndex++, cy,
                                        cWidth, cHeight);
                            }
                        }
            ...
            ArrayList<ArrayList<CompoInfo>> lst = new ArrayList<ArrayList<CompoInfo>>();
            ArrayList<CompoInfo> lhp = new ArrayList<CompoInfo>();
            lst.add(lhp);
            ...
            // 頭をあわせる
            for (int i = 0; i <= lst.size() - 1; i++) {
                ArrayList<CompoInfo> lh = lst.get(i);
                if (lh.size() > 0) {
                    if (xs[i] < lh.size() - 1) {
                        CompoInfo ci1 = lh.get(0);
                        int mxcal = ci1.rect.x;
                        boolean isleadercheck = true;
                        for (int j = xs[i] + 1; j <= lh.size() - 1; j++) {
                            CompoInfo ci2 = lh.get(j);
            ...
            while (minpos >= 0) {
                ...
                for (int i = 0; i <= lst.size() - 1; i++) {
                    ...
                    if (xs[i] < lh.size() - 1) {
                        // isCheck = true;
                        CompoInfo ci2 = lh.get(xs[i] + 1);
                        int mins = ci2.leader;
                        if (xs[i] >= 0) {
                            CompoInfo ci = lh.get(xs[i]);
