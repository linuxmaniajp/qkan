...
public class VRLabelContainer extends JPanel implements
    ...
    public void paintComponent(Graphics g) {
        ...
        if (getLabelWidth() > 0) {
            ...
            JLabel renderer = getLabelRenderer();
            if (renderer != null) {
                // if (lines.length > 0) {
                // 描画用ラベルに転写
...
//                int drawX = 0;
                ...
                // int lh = is.top - labelMargin * 2;
                // int keepH = Math.min(lh, h - labelMargin * 2);
                int keepH = h - is.bottom - is.top;
                drawY = is.top - 1;// 0;

//                FontMetrics fm = getFontMetrics(getFont());
                // TODO gh
                renderer.setBounds(0, 0, keepW, keepH);// fm.getHeight());
                // renderer.setBounds(0, 0, keepW, fm.getHeight());
    ...
    /**
     * ラベルの垂直方向の文字揃え を設定します。
     * 
     * @param verticalAlignment ラベルの垂直方向の文字揃え
     */
    ...
    /**
     * デフォルトのレイアウトマネージャを生成して返します。
     * @return デフォルトのレイアウトマネージャ
     */
    protected LayoutManager createLayout(){
        return new BoxLayout(this, BoxLayout.X_AXIS); 
    }
