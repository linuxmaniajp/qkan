...
public class VRTableOrderBySorter implements VRTableSortable {
    ...
    public void sort(VRSortableTableModelar model, int[] columnIndexs, int[] orders) {
        List<VRTableOrderBySortIndexItem> sortIndexs = null;
        for (int i = 0; i < columnIndexs.length; i++) {
            int columnIndex = columnIndexs[i];
            int order = orders[i];
            if (sortIndexs != null) {
                Iterator<VRTableOrderBySortIndexItem> itSortIndex = sortIndexs.iterator();
     ...
     * @return ソート結果
     */
    protected List<VRTableOrderBySortIndexItem> getSortIndexItem(TableModel model, int columnIndex) {
        List<VRTableOrderBySortIndexItem> sortIndexs = new ArrayList<VRTableOrderBySortIndexItem>();
        int count = 0;
