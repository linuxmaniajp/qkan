...
public class ACDateUtilities {
    ...
    /**
     * 期間2が期間1を包含し、かつ両期間の終了が同日であることをあらわす期間重複定数です。
     * <pre>
     * □□■期間１■□□<br />
     * □■期間２□■□□<br />
     * </pre>
     */
    public static final int DUPLICATE_SECOND_INCLUDE_FIRST_EQUALS_BOTH_END = 6;
    /**
     * 期間1の開始と期間2の終了が同日であることをあらわす期間重複定数です。
     * <pre>
     * □□□□□■期間１■□<br />
     * □■期間２■□□□□□<br />
     * </pre>
     */
    public static final int DUPLICATE_FIRST_BEGIN_AND_SECOND_END_ON_ONE_DAY = 7;
    /**
     * 期間1が期間2を包含し、かつ両期間の終了だけが同日であることをあらわす期間重複定数です。
     * <pre>
     * □■期間１□■□□<br />
     * □□■期間２■□□<br />
     * </pre>
     */
    public static final int DUPLICATE_FIRST_INCLUDE_SECOND_EQUALS_BOTH_END = 8;
    /**
     * 期間1の終了と期間2の開始が同日であることをあらわす期間重複定数です。<br />
     * <pre>
     * □■期間１■□□□□□<br />
     * □□□□□■期間２■□<br />
     * </pre>
     */
    public static final int DUPLICATE_FIRST_END_AND_SECOND_BEGIN_ON_ONE_DAY = 9;
    /**
     * 期間1が期間2を包含し、かつ両期間の開始だけが同日であることをあらわす期間重複定数です。
     * <pre>
     * □■期間１□■□□<br />
     * □■期間２■□□□<br />
     * </pre>
     */
    public static final int DUPLICATE_FIRST_INCLUDE_SECOND_EQUALS_BOTH_BEGIN = 10;
    /**
     * 期間2が期間1を包含し、かつ両期間の開始だけが同日であることをあらわす期間重複定数です。
     * <pre>
     * □■期間１■□□□<br />
     * □■期間２□■□□<br />
     * </pre>
     */
    public static final int DUPLICATE_SECOND_INCLUDE_FIRST_EQUALS_BOTH_BEGIN = 11;
    ...
    public static int getDuplicateTermCheck(Date span1Begin, Date span1End,
        ...
        if (beginDif > 0) {
            ...
            if (getDifferenceOnDay(span1End, span2End) == 0) {
                // 期間1の終了=期間2の終了
                return DUPLICATE_SECOND_INCLUDE_FIRST_EQUALS_BOTH_END;
            }
            ...
            if (getDifferenceOnDay(span1Begin, span2End) == 0) {
                // 期間1の開始=期間2の終了
                return DUPLICATE_FIRST_BEGIN_AND_SECOND_END_ON_ONE_DAY;
            }
        } else if (beginDif < 0) {
            // 期間1の開始＜期間2の開始
            if (getDifferenceOnDay(span1End, span2End) > 0) {
            ...
            if (getDifferenceOnDay(span1End, span2End) == 0) {
                // 期間1の終了=期間2の終了
                return DUPLICATE_FIRST_INCLUDE_SECOND_EQUALS_BOTH_END;
            }
            ...
            if (getDifferenceOnDay(span1End, span2Begin) == 0) {
                // 期間1の終了=期間2の開始
                return DUPLICATE_FIRST_END_AND_SECOND_BEGIN_ON_ONE_DAY;
            }
        } else {
            // 期間1の開始＝期間2の開始
            if (getDifferenceOnDay(span1End, span2End) > 0) {
                // 期間1の終了＞期間2の終了
                return DUPLICATE_FIRST_INCLUDE_SECOND_EQUALS_BOTH_BEGIN;
            }
            if (getDifferenceOnDay(span1End, span2End) < 0) {
                // 期間1の終了＜期間2の終了
                return DUPLICATE_SECOND_INCLUDE_FIRST_EQUALS_BOTH_BEGIN;
            }
            if (getDifferenceOnDay(span1End, span2End) == 0) {
                // 期間1の終了＝期間2の終了
     ...
     * @return 週数
     */
    public static int getDayOfWeekInMonth(Date x) {
        getCalX().setTime(x);
        return getCalX().get(Calendar.DAY_OF_WEEK_IN_MONTH);
    }
    /**
     * 年を返します。
     * 
