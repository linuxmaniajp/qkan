...
//    }
    ...
    public static void addFormat(ACChotarouXMLWriter pd, String formatID,
        ...
        if (processer instanceof ACPDFCreatable) {
            ...
            if (processer instanceof ACFrameEventDebugger) {
                // フォーマットのパスをログ出力
                VRLogger.log(((ACFrameEventDebugger) processer)
                        .getPrintLogLevel(), "addFormat: " + path);
            }
        } else {
    ...
    /**
     * 擬似的にセパレータとして使用するグリッド項目をXML出力します。
     * <p>
     * 右寄せで出力します。桁数制限は行いません。
     * </p>
     * 
     * @param pd 印刷管理クラス
     * @param target グリッド項目名
     * @param value 出力値
     * @throws Exception 処理例外
     * @return 出力したか
     */
    public static boolean setSeparatedGridValue(ACChotarouXMLWriter pd,
            String target, Object value) throws Exception {
        return setSeparatedGridValue(pd, target, value, -1);
    }
     ...
     */
    public static boolean setSeparatedGridValue(ACChotarouXMLWriter pd,
            String target, Object value, int columns) throws Exception {
        return setSeparatedGridValue(pd, target, value, columns,
     ...
     * 擬似的にセパレータとして使用するグリッド項目をXML出力します。
     * <p>
     * 隠蔽される引数gridRowNameConnectTextは".h1.w"で代用します。
     * </p>
     * 
     * @param pd 印刷管理クラス
     * @param target グリッド項目名
     * @param value 出力値
     * @param columns 桁数
     * @param alignment 文字揃え
     * @throws Exception 処理例外
     * @return 出力したか
     */
    public static boolean setSeparatedGridValue(ACChotarouXMLWriter pd,
            String target, Object value, int columns, int alignment)
            throws Exception {
        return setSeparatedGridValue(pd, target, value, columns, alignment,
                ".h1.w");
    }
     ...
     * @param columns 桁数
     * @param alignment 文字揃え
     * @param gridRowNameConnectText グリッド名と列番号の間に挿入する帳票定義体ID接続子
     * @throws Exception 処理例外
     * @return 出力したか
     */
    public static boolean setSeparatedGridValue(ACChotarouXMLWriter pd,
            String target, Object value, int columns, int alignment,
            final String gridRowNameConnectText) throws Exception {
        String val;
        if (value == null) {
            val = "";
        } else {
        ...
        if (len < columns) {
            ...
            case SwingConstants.RIGHT:
                beginOffset = columns - len;
                endOffset = columns;
                break;
            case SwingConstants.CENTER:
                beginOffset = (columns - len) / 2;
                endOffset = beginOffset + len;
                break;
            default:
                beginOffset = 0;
                endOffset = len;
            ...
            // 前方空白
            for (int i = 0; i < beginOffset; i++) {
                setValue(pd, target + gridRowNameConnectText + (i + 1), "");
            }

            // 印字
            for (int i = beginOffset; i < endOffset; i++) {
                setValue(pd, target + gridRowNameConnectText + (i + 1),
                        new String(values, i - beginOffset, 1));
            }

            // 後方空白
            for (int i = endOffset; i < columns; i++) {
                setValue(pd, target + gridRowNameConnectText + (i + 1), "");
            }
        } else {
            int beginOffset = 0;
            if (len > columns) {
                ...
                switch (alignment) {
                case SwingConstants.RIGHT:
                    beginOffset = len - columns;
                    break;
                case SwingConstants.CENTER:
                    beginOffset = (len - columns) / 2;
                    len = beginOffset + columns;
                    break;
                default:
                    len = columns;
                }
            }
            for (int i = beginOffset; i < len; i++) {
                setValue(pd, target + gridRowNameConnectText + (i + 1),
