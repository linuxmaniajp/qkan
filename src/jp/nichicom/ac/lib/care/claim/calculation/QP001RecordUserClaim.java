    ...
    */
    ...
    private void parseType(VRMap claim, String[] kohiType,QP001Manager manager) throws Exception {
        ...
        if(ACCastUtilities.toInt(claim.get("701029"),0) != 0){
        	...
        	//[H20.5 法改正対応] fujihara add end
            // 2006.05.09 edit end
        	// [H30.4改正対応][Yoichiro Kamei] 2018/4/15 add - begin
        	else if ("7157,7158,7196".indexOf(styleNo) != -1) {
        		int[] costs = unitCalc.getRecuperationCost(claim);
        		//特定治療費の自己負担が存在する
        		if (costs[0] != 0){
        			setServiceData("特定治療費(介護医療院)利用者負担",costs[0],claim, false);
        		}
        		//特別診療費の自己負担が存在する
        		if (costs[1] != 0){
        			setServiceData("特別診療費利用者負担",costs[1],claim, false);
        		}
        	}
        	// [H30.4改正対応][Yoichiro Kamei] 2018/4/15 add - end
        }
