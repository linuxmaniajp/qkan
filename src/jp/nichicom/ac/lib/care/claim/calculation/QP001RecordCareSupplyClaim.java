...
//    }
    ...
    protected void parse(VRMap base,int recordKubun, String lawNo, String kubun) throws Exception {
        ...
        // 請求情報区分コード2桁
        // 法別番号=0 OR 12→（7111の場合）01-居宅・施設サービス 02-居宅介護支援
        //                   （7113の場合）05-訪問型・通所型・その他の生活支援サービス費 06-介護予防ケアマネジメント費
        // 法別番号=それ以外→0固定
        // 【解説】
        // 保険請求、または公費請求で法別番号が12(生保)の場合→01-居宅・施設サービス 02-居宅介護支援
        // 総合事業の場合は、05-訪問型・通所型・その他の生活支援サービス費 06-介護予防ケアマネジメント費
...
// 2018/2/19 [CCCX: 04509][Yoichiro Kamei] add - begin 生保単独の特定入所者レコードのみの公費に対応
            int seikyu = 0;
            int nyusho = 0;
            switch(kohiPosition){
            case 0:
                seikyu = ACCastUtilities.toInt(base.get("201040"), 0) // 公費１
                        + ACCastUtilities.toInt(base.get("201042"), 0) //(合計情報 公費1)緊急時施設療養費請求額8桁
                        + ACCastUtilities.toInt(base.get("201043"), 0); //(合計情報 公費1)特定診療費請求額8桁
                nyusho = ACCastUtilities.toInt(base.get("201044"), 0); //(合計情報 公費1)特定入所者介護サービス費等請求額8桁
                        break;
            case 1:
                seikyu = ACCastUtilities.toInt(base.get("201046"), 0) // 公費２
                        + ACCastUtilities.toInt(base.get("201048"), 0) //(合計情報 公費2)緊急時施設療養費請求額8桁
                        + ACCastUtilities.toInt(base.get("201049"), 0); //(合計情報 公費2)特定診療費請求額8桁
                nyusho = ACCastUtilities.toInt(base.get("201050"), 0); //(合計情報 公費2)特定入所者介護サービス費等請求額8桁
                        break;
            case 2:
                seikyu = ACCastUtilities.toInt(base.get("201052"), 0) // 公費３
                        + ACCastUtilities.toInt(base.get("201054"), 0) //(合計情報 公費3)緊急時施設療養費請求額8桁
                        + ACCastUtilities.toInt(base.get("201055"), 0); //(合計情報 公費3)特定診療費請求額8桁
                nyusho = ACCastUtilities.toInt(base.get("201056"), 0); //(合計情報 公費3)特定入所者介護サービス費等請求額8桁
                        break;
            }
            if (seikyu == 0 && nyusho != 0) {
            	// 特定入所者介護サービス費のみの場合は通常のサービス費用の設定は不要なためスキップ
            	return;
            }
...
// 2015/5/12 [Yoichiro Kamei] mod - begin 公費関連見直し
            ...
            if (_101007.containsKey(key)) {
                existsFlg = true; 
                //同じ被保険者番号で公費負担者番号が異なる場合は２件にする
                //下記の対応
                //(資料)介護給付費請求書等の記載要領について(P3)
                //ただし、市町村合併等により被保険者等一人につき二か所の生
                //活保護又は中国残留邦人等公費に係る介護支援給付実施機関へ請
                //求を行う場合には、二件と記載すること。
                String payerNo = "";
                switch (kohiPosition) {
                case 0:
...
//                        + ACCastUtilities.toInt(base.get("201043"), 0) //(合計情報 公費1)特定診療費請求額8桁
//                        );
//                //[Shinobu Hitaka] 2014/03/16 edit - start 合計値があわない
////                set_101012(get_101012()
////                        + ACCastUtilities.toInt(base.get("201052"), 0) // 公費３
////                        + ACCastUtilities.toInt(base.get("201054"), 0) //(合計情報 公費3)緊急時施設療養費請求額8桁
////                        + ACCastUtilities.toInt(base.get("201055"), 0) //(合計情報 公費3)特定診療費請求額8桁
...
//                        + ACCastUtilities.toInt(base.get("201047"), 0) //(合計情報 公費2)本人負担額8桁
//                        );
//                //[Shinobu Hitaka] 2014/03/16 edit - end 合計値があわない
//                        break;
//            }

// 2018/2/19 [CCCX: 04509][Yoichiro Kamei] mod - begin 生保単独の特定入所者レコードのみの公費に対応
//            int seikyu = 0;
//            switch(kohiPosition){
//            case 0:
//                seikyu = ACCastUtilities.toInt(base.get("201040"), 0) // 公費１
