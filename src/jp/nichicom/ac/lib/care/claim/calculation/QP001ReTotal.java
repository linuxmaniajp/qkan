...
public class QP001ReTotal {
 ...
 // 2014/12/24 [Yoichiro Kamei] add - begin 住所地特例対応
    /** 明細情報（住所地特例）レコード(18) */
    private VRList detailJushotiTokurei = new VRArrayList();
...
// 2015/04/13 [Yoichiro Kamei] add - end
        ...
        for (int i = 0; i < nursing.size(); i++) {
            ...
            if (QP001SpecialCase.isShisetsuDiscriminationNo(toString(map,
                ...
                if (QP001SpecialCase.isSeihoOnly(toString(map, "801006"))) {
                    ...
                    // 種類種別(1.食費　6.多床室(老健・療養等)　7.多床室(特養等))
                    int serviceKind = toInt(map, "801037");
                    // 1.食費　6.多床室(老健・療養等)　7.多床室(特養等)以外は算定しない
    ...
    private void parseType() throws Exception {
        ...
        for (int i = 0; i < detail.size(); i++) {
            ...
            */
            String serviceCodeKind = ACCastUtilities.toString(map.get("301007"), "");
            // 福祉用具
...
// 2015/6/18 [Shinobu Hitaka] mod - begin 公費関連見直し（10感染症の場合に公費給付率を初期化）
...
// 2015/6/18 [Shinobu Hitaka] mod - end 
            ...
            if (!kohiEnd && (getKohiRate(3) != 0)
                ...
                if(roukenFlg){
                    //公費1=10感染症、公費2=老健一部公費の場合、請求額を調整する
                    //if ("52".equals(toString(map, "701007")) && "1001".equals(getKohiType(1))){
                    if ("1001".equals(getKohiType(1))){
                        if (base.size() > 0) {
    ...
    private void commitDiagnosis(VRMap map) throws Exception {
        ...
        // 公費1の適用あり
// 2015/6/18 [Shinobu Hitaka] mod - begin 公費関連見直し
//        if (getKohiRate(1) != 0) {
...
// 2015/6/18 [Shinobu Hitaka] add - begin 公費関連見直し
