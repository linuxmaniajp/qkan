...
public class ACKanaSendTextField extends ACTextField {
    ...
    protected Document createDocument() {
        return new ACKanaSendTextFieldDocument(this);
    }
     ...
     */
    protected class ACKanaSendTextFieldDocument extends ACTextFieldDocument {
         ...
         */
        public ACKanaSendTextFieldDocument(AbstractVRTextField textField) {
            super(textField);
...
//                    tmp = deleteMissingKana(imeString, missingString, pos);
            ...
            //最大文字数チェック
            int maxLen = getTextField().getMaxLength();
            if (maxLen > 0) {
                int txtLen = getTextField().getText().length();
                if (maxLen < txtLen) {
                    return;
                }
                if (maxLen < txtLen + str.length()) {
                    //間引く
                    str = str.substring(0, maxLen - txtLen);
                }
            }
            ...
            if (getKanaField() != null) {
                String sendText = imeString;
                //全角スペース対応
                if ("　".equals(str)&&"".equals(sendText)) {
                    sendText = str;
                }
                if (!"".equals(sendText)) {
                    if (getTextField().hasFocus()) {
                        while (true) {
                            //擬似ループ
                            String conved = convToHiragana(sendText);
                            chrType = getKanaField().getCharType();
                            if ((chrType != null) && (!chrType.isMatch(conved))) {
                                // 許可文字種別のチェック
                                break;
                            }
                            String nowText = getKanaField().getText();
                            maxLen = getKanaField().getMaxLength();
                            if (maxLen > 0) {
                                int txtLen = nowText.length();
                                if (maxLen < txtLen) {
                                    break;
                                }
                                int convLen = conved.length();
                                if (maxLen < txtLen + convLen) {
                                    // 間引く
                                    conved = conved.substring(0, maxLen
                                            - txtLen);
                                }
                            }
                            getKanaField().setText(nowText + conved);
                            break;
                        }
                        imeString = "";
                        inputString = "";
