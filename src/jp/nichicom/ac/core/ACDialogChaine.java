 ...
 */
public class ACDialogChaine {
    private static ACDialogChaine singleton;
     ...
     * @return インスタンス
     */
    public static ACDialogChaine getInstance() {
        if (singleton == null) {
            singleton = new ACDialogChaine();
        }
        return singleton;
    }

    /**
     * 登録されているすべてのダイアログを解放します。
     */
    public void closeAll() {
        Iterator it = iterator();
        while (it.hasNext()) {
            JDialog spl = (JDialog) it.next();
            spl.dispose();
        }
    }
     ...
     *        &lt; 0 || index &gt;= size())</tt>.
     */
    public JDialog get(int index) {
        return (JDialog) getList().get(index);
    }
     ...
     * @return 最後に登録したダイアログ
     */
    public JDialog getLastItem() {
        List list = getList();
        if(list.size()>0){
            return (JDialog)list.get(list.size()-1);
        }
        return null;
    }
    
    /**
     * クラスのインスタンスのうち最後に登録したダイアログを返します。
     * @param type フィルタクラス
     * @return 最後に登録したダイアログ
     */
    public JDialog getLastItem(Class type) {
        List list = getList();
        int end= list.size();
     ...
     *        &lt; 0 || index &gt;= size())</tt>.
     */
    public JDialog remove(int index) {
        return (JDialog) getList().remove(index);
    }
     ...
     */
    public void add(int index, JDialog element) {
     ...
     */
    public int lastIndexOf(JDialog o) {
     ...
     */
    public boolean contains(JDialog o) {
     ...
     * o.equals(e))</tt>, if
     * the collection contains one or more such elements. Returns <tt>true</tt>
     * if the collection contained the specified element (or equivalently, if
     ...
     */
    public boolean remove(JDialog o) {
     ...
     *             <tt>(index &lt; 0 || index &gt;= size())</tt>.
     */
    public Object set(int index, JDialog element) {
